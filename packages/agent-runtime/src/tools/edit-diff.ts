 ...
 * 精确替换与 diff 计算（移植）
 *
 * 来源：`vendor/pi/packages/agent/src/harness/tools/edit-diff.ts`
 * （与上游 `pi/packages/coding-agent/src/core/tools/edit-diff.ts` 同源，逐行一致）。
 *
 * 为什么是「复制 + 改写」而不是 import：vendor 的 package.json `exports` 只露出
 * `.` / `./node` 等入口，`harness/tools/edit-diff.ts` 不在导出面内；
 * 而 AGENTS.md 明确禁止引入 `pi-coding-agent`。按硬约束走复制路径。
 *
 * 该文件提供的能力（Edit 工具依赖）：
 *  - BOM / CRLF 归一化，模型给的行尾与磁盘行尾不一致时仍能命中
 *  - 精确匹配优先、fuzzy 匹配兜底（尾随空白 / 弯引号 / 破折号 / 特殊空格）
 *  - 多段 disjoint 编辑共用同一份原文匹配，重叠即报错
 *  - 变更行号 + 可读 diff + 标准 unified patch
 *
 * 依赖：npm `diff@8`（上游 coding-agent 同样直接依赖该包）。
 *
