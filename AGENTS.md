...
- **溯源台账已落地（符号层 + 蓝图声明）**：`traceability.ts` 纯函数 + 只读，
  复用 repo-analyzer 已产出的缓存清单（`last_manifest.json` / `last_symbols.json`）
  零额外解析成本（`MIGRATION.md` §31）。**符号层只 WARN 不 FAIL**
  （匹配不到可能是幻觉 / 缓存过期 / 引用的是文档或工具名）；**蓝图 `associatedFiles`
  不存在直接 FAIL**。旧版 `ValidateBlueprintTool` 已标记仅归档。
  页面格式资产化（P1-2）、标题诊断信号（P2）未开始，见 `plan.md`。
