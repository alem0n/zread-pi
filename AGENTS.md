...
| 版本守卫（v1.13.0：按主版本隔离数据目录） | `~/.zread-pi/version` 与 `<repo>/.zread-pi/version` 记录「由哪个版本创建」；CLI 启动时检查（兼容 = **主版本号相同**）：目录不存在 → 静默创建；目录存在但无 version 文件 / 主版本不同 → 把目录整体备份为 `<dir>_bak`（占用则 `-2`/`-3`）后重建，并在 stderr 给出提示。纯逻辑在 `packages/utils/src/version-guard.ts`，CLI 包装在 `apps/cli/src/commands/version-guard.ts`（`runApp` 与 `history` 命令调用，接管终端前执行；`ZREAD_PI_VERSION_GUARD=0` 跳过）。语言提示用 `loadConfigLanguageSync`（不做完整校验，旧/残缺配置也能给出正确语言） |
...
| `test:version-guard` | 版本守卫：版本号解析与兼容判定（主版本相同 / 非法值不兼容）、version 文件读写、备份路径命名（`_bak` / `-2` / `-3`）、`ensureVersionGuard` 四条路径（首次安装 / 兼容幂等 / 无版本文件 → 备份 / 跨主版本 → 备份，旧数据完整保留在备份里）；CLI 包装层（stderr 提示含备份路径 / 兼容时无输出 / 首次静默 / `ZREAD_PI_VERSION_GUARD=0` 跳过 / `repo:false` 不碰仓库目录 / 提示语言随旧配置） | 39/39 + 17/17                           |
...
| 版本守卫（`packages/utils/src/version-guard.ts`、`apps/cli/src/commands/version-guard.ts`、调用点 `app.ts` / `index.ts`） | `bun run typecheck` + `bun run test:version-guard` + `bun run test:tui` + `bun run test` | 兼容判定口径（主版本）或备份命名（`_bak`）改动必须补 `test:version-guard` 断言；新增 CLI 调用点必须确认在 TUI 接管终端**之前**执行，且 spawn 型测试已设 `ZREAD_PI_VERSION_GUARD=0`（否则临时家目录会被当成不兼容数据备份掉） |
...
- `~/.zread-pi/version` 与 `<repo>/.zread-pi/version`：版本标记（v1.13.0 起，版本守卫用；见 §1.1）。不兼容（主版本不同 / 无标记）时整个目录会被备份为 `<dir>_bak` 后重建，**旧数据完整保留在备份里**。`ZREAD_PI_VERSION_GUARD=0` 可跳过该检查。
