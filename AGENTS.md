...
- **内容密度门已落地，但交付闸门 `verify-wiki` 尚未实现**（`plan.md` 的 P0-2）：
  `quality.contentGate` 的度量 / 拦截 / 降级落盘 / 配置界面 / 生成页 ⚠ 标记已完成（`MIGRATION.md` §29），
  但**没有一条「判定本次生成是否达标」的独立命令**——判定仍散在 `generate-wiki` 的 `fileExists` +
  路径救援 + 内容门里。`quality.verifyAfterGenerate` 配置字段已预留（缺省 `false`，**当前未接线**，
  生成完成后不会自动跑校验）。溯源台账校验（P1）、页面格式资产化（P1-2）、标题诊断信号（P2）
  亦未开始，见 `plan.md`。
