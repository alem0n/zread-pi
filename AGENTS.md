...
| 版本守卫（v1.13.0：按主版本隔离数据目录） | `~/.zread-pi/version` 与 `<repo>/.zread-pi/version` 记录「由哪个版本创建」；CLI 启动时检查（兼容 = **主版本号相同**）：目录不存在 → 静默创建；目录存在但无 version 文件 / 主版本不同 → 把目录备份为 `<dir>_bak`（占用则 `-2`/`-3`）后重建，并在 stderr 给出提示。**目录被别的进程当作 cwd 时整体重命名会被 Windows 拒绝（EBUSY/EPERM），此时自动降级为「逐条目迁入新备份目录」**（结果带 `degraded: true`），并给出降级说明；两种方式都失败时不再静默吞掉，改为 stderr 告警。纯逻辑在 `packages/utils/src/version-guard.ts`，CLI 包装在 `apps/cli/src/commands/version-guard.ts`（`runApp` 与 `history` 命令调用，接管终端前执行；`ZREAD_PI_VERSION_GUARD=0` 跳过）。语言提示用 `loadConfigLanguageSync`（不做完整校验，旧/残缺配置也能给出正确语言） |
...
| `test:version-guard` | 版本守卫：版本号解析与兼容判定（主版本相同 / 非法值不兼容）、version 文件读写、备份路径命名（`_bak` / `-2` / `-3`）、`ensureVersionGuard` 四条路径（首次安装 / 兼容幂等 / 无版本文件 → 备份 / 跨主版本 → 备份，旧数据完整保留在备份里）**+ 降级路径（目录被别的进程 chdir 占用 → 逐项迁出，`degraded` 标记、数据完整）**；CLI 包装层（stderr 提示含备份路径 / 兼容时无输出 / 首次静默 / `ZREAD_PI_VERSION_GUARD=0` 跳过 / `repo:false` 不碰仓库目录 / 提示语言随旧配置 / **降级提示可见**） | 46/46 + 22/22                           |
...
| `test:trajectory`  | 轨迹模型层（92 项）：replay 折叠（turn=agent / group 归属 / 请求统一编号 / 失败归属 / partial）、**并发归属（session id：交错事件流不互相吞 / agent_end 不清空别的 Agent 的在途消息 / 旧日志无 sessionId 回退 key）**、**turn 的 sessionId 唯一性**、layout、timeline 四模式投影 + 选区过滤、**session 隐藏后时间线重新投影**、搜索索引（多词交集 / 增量更新）、虚拟化窗口数学 + 稳定 key、格式化函数；store 层（46 项）：writer/reader 往返、beforeSeq/afterSeq 分页、损坏行跳过、保留期清理（删最早开始的 run）、残留 running 自愈 interrupted、run.json 写入串行（end 终态不被挂起写覆盖）、withRunLog 自动建 run / 复用 / 失败记 failed、runId 校验与列表排序 | 92/92 + 46/46                          |
...
| 版本守卫（`packages/utils/src/version-guard.ts`、`apps/cli/src/commands/version-guard.ts`、调用点 `app.ts` / `index.ts`） | `bun run typecheck` + `bun run test:version-guard` + `bun run test:tui` + `bun run test` | 兼容判定口径（主版本）或备份命名（`_bak`）改动必须补 `test:version-guard` 断言；**降级 / 失败告警语义改动也必须补断言**（模拟 chdir 占用）；新增 CLI 调用点必须确认在 TUI 接管终端**之前**执行，且 spawn 型测试已设 `ZREAD_PI_VERSION_GUARD=0`（否则临时家目录会被当成不兼容数据备份掉） |
...
- `~/.zread-pi/version` 与 `<repo>/.zread-pi/version`：版本标记（v1.13.0 起，版本守卫用；见 §1.1）。不兼容（主版本不同 / 无标记）时整个目录会被备份为 `<dir>_bak` 后重建，**旧数据完整保留在备份里**。目录被别的进程当作 cwd 时整体重命名会被 Windows 拒绝，此时降级为逐项迁出（数据同样完整进备份）。`ZREAD_PI_VERSION_GUARD=0` 可跳过该检查。
...
- **版本守卫在跨主版本升级时会备份旧数据目录**（v1.13.0 起，有意为之）：`~/.zread-pi` 与 `<repo>/.zread-pi` 没有 `version` 标记或主版本不同时，整个目录会被改名为 `<dir>_bak` 并重建。**首次运行新主版本 = 现有配置与产物被移到 `_bak`**，用户需手动迁回所需文件。`_bak` / `_bak-N` 与 `.zread-pi` 同级，**不会被用户仓库的 `.gitignore` 覆盖**（本仓库自己的 `.gitignore` 已加 `.zread-pi_bak/` 规则）。
  - **目录被占用时走降级路径**：若该目录正被别的进程当作 cwd（常见于 AI 编程环境等常驻进程），Windows 拒绝整体重命名，守卫改为把目录内条目逐个迁入新的 `_bak`，旧数据同样完整保留。此时会额外提示「降级备份」，建议关闭占用程序后重跑以获得干净结构。两种方式都失败时**不再静默**，会在 stderr 明确告警并保持旧数据不动。
