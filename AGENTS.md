...
与 `README.md`（怎么用）、`RULES.md`（包结构与逐包修改要点）、`DESIGN.md`（视觉规范）配合使用；冲突时以本文件为准。
...
| 首尾机制跟随 harness 升级，而不是原样搬运                   | 裸 loop 的 `shouldStopAfterTurn` / 手写 `transformContext` 压缩 / `agent.steer()` 收尾提示换成 harness 的 `before_run` / `before_run_end` / `before_tool`：轮数硬顶→**token 预算**（usage 事件/ledger 权威累计）、一次性提示→**两段式提示**、返回 true 停止→**不返回 followUp 终止**、`error_max_turns`→**预算耗尽强制交卷 + 编排层判页失败** |
...
| 钩子映射到 harness 的 `before_run` / `before_run_end` / `before_tool` / `after_tool` | 旧 `PreToolUse` / `PostToolUse` 与 UI 进度事件零改动；首尾机制（预算提示/终止/熔断）挂在同一套钩子上                                                                                                                                                                                                                                                                                                                                                                                              |
...
| 图片处理管线（第十二步，移植 pi coding-agent）              | `Read` 不再原样回传图片：格式归一到 png/jpeg/gif/webp（BMP/TIFF 等转 PNG）、超过 2000×2000 或 4.5MB base64 时缩放（PNG/多档 JPEG 取小、逐级 0.75 缩）、回传坐标换算提示；Worker 线程跑 WASM，失败回退进程内。仅当模型支持图片时才处理；无法处理时降级为文本说明（不报 tool error）。实现：`agent-runtime/src/tools/image/**` |
| 目标仓库自述注入系统提示（第十二步）                       | 生成文档前读 `AGENTS.override.md` / `AGENTS.md` / `AGENTS.MD` / `CLAUDE.md` / `CLAUDE.MD`（全局 `~/.zread-pi` + 目标仓库根），按 pi 的 `<project_context>` 格式拼进蓝图/页面 Agent 的系统提示；单文件 64 KiB 截断。与上游差异：不向上遍历父目录 |
| 文风纪律（humanizer）两层机制（第十三步）                  | 第 1 层预防（`polish.enabled`，默认开）：按 `doc_language` 选 `humanizer.en.md` / `humanizer.zh.md`（各 60~80 行、头部注明来源），拼进蓝图/页面 Agent 系统提示、排在 `<project_context>` 之后，零额外调用；第 2 层兜底（`polish.mode=full`）：每页落盘后跑轻量 polish Agent（Read/Edit/Ls + 独立小预算，提示词 = 纪律 + Embedded mode），失败不判页失败、Mermaid 被改坏回滚。配置界面 `/config/polish` |
| 页面内容密度门（`quality.contentGate`，v1.14.0） | 迁移自 lecture-to-notes 的 `density_gate`：把「这篇页面是不是干瘪 TL;DR」变成机械指标（散文字符 / 标题层级 / Mermaid / 代码块 / Sources / 重复句首），在 `write_page` 的 Mermaid 校验之后拦截。`enforce` 未达标返回 `is_error` + 「当前 N / 下限 M」常驻反馈让模型重写；预算用尽后 `writeDegradedPage()` best-effort 落盘（`gate.mode='enforce-degraded'`，**计成功 + 告警**，不判页失败——「生成永不悬挂」）；`warn`（缺省）只报告不拦截。反注水：门限是下限不是目标、代码块是软建议（源里没代码时 0 是正确答案）。配置界面 `/config/quality` |
| 浏览站跟随 CLI 界面语言（logview 网页中文化）           | CLI 有中英双语但浏览站（轨迹页）原为英文硬编码。服务端新增 `GET /api/i18n`（`resolveBrowseLocale()` 读 `language` 配置并归一化），前端 `apps/browse/src/i18n/` 是**独立于 CLI 的第二套字典**（两套应用文案不重合），`I18nProvider` 启动时取一次语言经 Context 下发。默认/回退 `en-US`（= 改造前文案，现有用户零变化）。数据层派生的标签（turn label / 「Initial System Prompt」等）不翻译 |
...
| 方案 C：pi 会话 = 唯一完整事实源（第三十步） | 轨迹内容双写（pi 会话 + 捕获层再抄一遍）有三个硬伤：截断失真（预览 512 / 输出 64KiB / 摘要 8KiB）、重复劳动（流式状态机 / 工具错误暂存 / request id 挂终态时序协议）、口径漂移（两份拷贝各自演进）。阶段 0–1 已让会话可读（探针 + `sessionRoot` → `JsonlSessionRepo` 落 `<runDir>/sessions/`）；本步做消费侧：内容全部从会话投影，`events.jsonl` 只留瘦业务事件（`agent_config` + `agent_end` + 每响应一条 `provider_request` + run/page/stage/section/scan）。`agent_config` 承载三个不在会话里的 harness 配置事实（systemPrompt 全文 / toolCatalog schema / tokenBudget）+ 会话 id；失败归属用 `keyToIdentity` 双键回退（sink 绑定只有 key）。旧 run 无会话目录 → 回退 `replayRunEvents`。**取舍**：浏览站轨迹页运行中无 partial 预览（TUI 生成页进度不受影响） |
| 配置写入加跨进程文件锁（第十二步）                         | `packages/utils/src/lockfile.ts`（`proper-lockfile`，`realpath:false`，ELOCKED 重试 10×20ms）包住 config.yaml / auth.json / tools-state.json / history 的「读-改-写」；config 额外临时文件 + rename 原子替换。锁失败按写入失败报错，不静默降级 |
| TUI stdout 接管（第十二步）                               | `apps/cli/src/tui/output-guard.ts`（移植 pi coding-agent）：接管 `process.stdout.write`，TUI 自身写入走放行窗口/ESC 控制序列直达原生 stdout，杂散写入进日志文件（pi 默认送 stderr，可用 `redirect` 配置）；`guarded-terminal.ts` 把 pi-tui 终端方法包进放行窗口；`zread-pi history` 的清单输出也走 `writeRawStdout` |
...
| 生成页底部展示用量合计（第十四步）                          | 生成文档界面最下方实时展示**目录 Agent + 全部页面 Agent** 的输入 token（输入侧总量 = 非缓存输入 + 缓存读 + 缓存写）/ 输出 token / 缓存读占比（无用量时整行不渲染）。合计**不是**共享计数器上的增量累加，而是渲染时对「每页自己的累计快照」做幂等 reduce：多页并发不会重复计数/丢更新。槽位分两段：`usage` = 本轮运行累计快照，`carryUsage` = 历史轮次结转；**重试/重新生成不清零**（成功 + 失败 + 重试的消耗都留在槽位里，合计 = carry + 本轮），同一页正在生成时忽略重复触发。失败页/失败目录带最后一次累计快照（`generate-wiki.ts` 记录、`create-agent.ts` 的 error 事件带上、mapper 再做 `event.usage ?? 旧值` 兜底）。口径来自 pi 的 usage ledger；**不含** `polish.mode=full` 的 polish Agent 用量 |
| 生成页逐 Agent 行 + 上下文占比（第十九步） | 生成页**每个条目右侧状态后**都显示四个指标：输入 token（输入侧总量）/ 输出 token / 缓存占比 / **上下文占比（已用/窗口）**；**完成 / 失败也照常显示**。目录不再是一行聚合：**每个 Agent 一行**（`规划主题` / `拟定标题 · 章节` / `精修标题 · 章节` / 缩编 subagent 缩进在目录行下），行内用量取 `agentUsage`（该 Agent **自己**的累计快照；`usage` 仍是目录级聚合，不混用），上下文已用 = 最近一次响应的 input+output+cacheRead+cacheWrite（与 pi compaction 同口径），窗口来自 `system/init` 新增的 `context_window`。带 `agentKey` 的 `complete`/`error` 只改行、不改目录整体状态（并修掉「首个 Agent 完成就 reload 并提前启动页面」的竞态）；业务判定「未产出工具」时由 `markAgentFailed` 把该行改为失败 |
...
- 第十四步起 `TokenUsage` 归并新增纯函数导出：`emptyTokenUsage / addTokenUsage / sumTokenUsage`（`packages/agent-runtime/src/usage.ts`，唯一归并口径；pi 的 `addUsage` 只作用于内部 `Usage` 形状且未从包根导出）；`CatalogEvent` / `ArticleEventPayload` 的 `usage` 语义补齐为「该 Agent 的累计快照」（**含失败事件**带上最后一次快照，见 §1.1）
- 首尾机制配置面：`budget.maxTokens / softRatio / forcedTurns / notices / outputTools`；`maxTurns` / `finalization` 保留为兼容字段（折算 token 预算）
...
- `RetryConfig` 新增**可选** `provider: { maxRetries, maxRetryDelayMs, timeoutMs }`（Provider 层重试，pi-ai 读 `Retry-After`）；`toRetryPolicy()` 对 `maxRetries <= 0` 返回**显式禁用**策略（不再回退 harness 默认 3 次），`toStreamOptions()` 负责下发 Provider 层配置（第十二步）
- 图片处理管线对外新增导出：`processImage / resizeImage / formatDimensionNote / convertImageBytesToPng / convertToPng / loadPhoton` + 类型（均为新增，旧调用点零改动）
...
- 第二十三步（内容密度门）：`AppConfig.quality: { contentGate: { enabled: boolean; mode: 'off'|'warn'|'enforce' }; verifyAfterGenerate: boolean }`（旧配置缺省 `{ enabled: true, mode: 'warn', verifyAfterGenerate: false }`，安全可启动；配置界面 `/config/quality` 维护）；`PageResult.gate?: ContentGateReport`（`{ passed, mode: 'warn'|'enforce'|'enforce-degraded', metrics, failures[], advisories[], durationMs }`，**新增可选字段**，门未通过**不改变** `success`）；`CatalogEvent` / `ArticleEventPayload` 新增**可选** `gate?: ContentGateReport`（`page_complete` / `page_error` 携带；`AgentResult` 不含）；orchestrator 新增导出 `evaluateContentGate` / `formatContentGateError` / `extractGateReport` / `resolveGateMode` / `CONTENT_GATE_SPECS` / `proseFloor` / `mermaidRequiredFor` / `codeRecommendedFor` / 类型 `ContentGateReport` / `ContentGateMetrics`；`createWritePageTool` 参数从 `BlueprintDetailLevel` 放宽为 `WritePageToolOptions | BlueprintDetailLevel`（新增可选 `contentGate`；旧调用点零改动），新增导出 `buildPageFrontmatter`；`PageWriteAttempt` 新增**可选** `content?` / `gateBlocked?`；CLI 新增 `PageStatus.gate?` 与纯函数渲染的行内 ⚠ 标记
- 第二十四步（交付闸门）：orchestrator 新增导出 `verifyWiki` / `parseSourceRefs` 与类型 `VerifyReport` / `VerifyCheck` / `VerifyStatus` / `VerifyGroup` / `VerifyWikiOptions`（**只读**：不写任何产物；`verify.json` 由 CLI / generate-wiki 集成落盘）；CLI 新增子命令 `zread-pi verify [-d <dir>] [--detail <档位>] [--enforce]`（`apps/cli/src/commands/verify.ts`；检查行格式 `<PASS|FAIL|SKIP> <group> <message>`，末尾 `OVERALL PASS/FAIL` + 退出码 0/1；输出走 stdout 接管）；`quality.verifyAfterGenerate`（既有字段，此前未接线）打开时 `generateWikiContent` 在 run 目录落 **独立文件** `<runId>/verify.json`（**不改 `RunMeta`**，trajectory replay 无感知）；i18n 新增 `cli.verify*` 与 `verify.*` 字段
- 第二十五步（溯源台账）：orchestrator 新增导出 `parseSourceRefs`（**从 verify-wiki 迁入，旧导入路径保留**——verify-wiki re-export）、`collectKnownSymbols` / `findUnresolvedSymbols` / `collectManifestPaths` / `isPathReal` / `countLines` / `checkAssociatedFiles` / `checkTraceability` 与类型 `SourceRef` / `AssociatedFileIssue` / `TraceabilityInput` / `TraceabilityResult`（纯函数 + **只读**：只读缓存清单与被引用的源文件，不写任何产物）；verify-wiki 的 `structure` 组新增 `associatedFiles` 存在性检查（**复活旧版 `validate_blueprint`**，页面维度），`traceability` 组从 3 项变 4 项（新增符号可溯，与 `Sources:` 正交、始终执行；符号缓存缺失 → SKIP）；`ValidateBlueprintTool` 标记为**仅归档**（与 `generate_blueprint` 同策略）
- 第二十六步（页面格式资产 + reader-first）：orchestrator 新增导出 `getPageFormat` / `formatPageFormat` / `withPageFormat` / `PAGE_FORMAT_TAG`（`agents/page-format.ts`）、`getReaderDiscipline` / `formatReaderDiscipline` / `withReaderDiscipline` / `READER_SELF_CHECK` / `READER_FIRST_TAG`（`agents/reader-first.ts`）、`checkPolishDiff` + 类型 `PolishDiffViolation`（`wiki/polish.ts`，纯函数：frontmatter / `Sources:` / Mermaid 三项 diff 断言，只许改散文）；新增提示词资产 `prompts/page-format.zh.md` / `.en.md` / `prompts/reader-first.zh.md` / `.en.md`（**两份同步改动，条数与编号一一对应**）；`buildPolishSystemPrompt` 追加 `READER_SELF_CHECK`；`PolishOutcome.reason` 新增 `structure-rollback`；`GenerateWikiOptions` 新增**可选** `language`（缺省 = `config.doc_language`）；`buildPagePrompt` 新增可选第 4 参数 `language`；`create-agent.ts` 的系统提示在 `withStyleDiscipline` 之后追加 `withReaderDiscipline`（共用 `polish.enabled` 开关）
- 第二十七步（标题诊断信号）：`prompts/titles.ts` 追加「诊断信号」段（对齐 lecture-to-notes `structure-reorder.md` 的 7 条：子层级编号 / 续接词 / 相邻同母题 / 字数悬殊 1/5 / 具体技术点 / 母题词反复 / 命名风格不统一 + 收敛优先策略 + 不得新增删除调换页面）；`createRefineSectionTitlesTool` 新增**可选** `expectedSlugs`（落盘前数量一致性自检：陌生 slug / 漏 slug → `is_error` + 具体差异，**不落盘**）与 `onResult`（重写率回调，阶段驱动器记 `titles` 命名 logger）；`applySectionTitles` 补字段不可变性实现注释（`page.title` 结构性只赋值项，无需比对校验）；`blueprint-stages.ts` 传入 `expectedSlugs` 并在成功路径记 `标题重写率：updated/total（rate%）`
- 第二十八步（移除「快速开始」基础分类）：`BASE_SECTIONS` zh/en 各删一条（3 → 2，`概览` / `核心架构` 两个基础分类）；`BLUEPRINT_DETAIL_SPECS` 的 sections **下限 -1**（low 3→2 / medium 4→3 / high 4→3 / max 4→3，因 `judgeQuantity` 判的是归一化后的 count）；`classify.ts` 数量目标与示例 JSON；`submit_sections` 的 JSDoc + schema description；`content-gate` 的 `OVERVIEW_SECTIONS` 删 `快速开始` / `quick start`（**该角色页面不再强制 Mermaid**，概览/核心架构仍强制）。旧 wiki.json 的快速开始分类仍可读取（`loadWikiBlueprint` 不校验集合），sync merge 的「既有分类必留」会原样保留（只是不再强补）
- 第二十九步（request id 透传，排障用）：适配层订阅 harness 的 `after_response` 钩子，经纯函数 `extractRequestId()` 从 provider **响应头**归一化提取 request id（兼容 `x-request-id` / `request-id` / `x-amzn-requestid`，大小写不敏感），挂在紧随其后的 `assistant`（`message_end`）事件上（`after_response` 在 `message_end` 之前触发且 1:1 配对，顺序可靠）；编排层写入 `message_end` 的可选字段落进 `events.jsonl`。均为**新增可选字段**（`SDKAssistantMessage.request_id?` / `MessageEndEvent.requestId?`），provider 未返回时缺省，旧日志无字段照常读取，replay / 布局 / UI 均不依赖；钩子只读不替换消息，事件时序与结果归类不变。**未接入 UI**（`CatalogEvent` 不新增字段，排障直接读日志）；browse-chat 的 `createMessage()` 路径未接入（走 `completeSimple`，无该钩子）
 - 第三十步（方案 C · 阶段 2/3：pi 会话成为唯一完整事实源）：轨迹内容（消息 / 工具 / 结果 / 用量 / 压缩摘要）全部从 pi 会话条目投影，捕获层退回纯业务事件——`create-agent.ts` 删除流式状态机与 `message_*` / `tool_*` / `retry` / `compact` / `status` / `agent_start` 的捕获，`agent_start` → 在 `system/init` 时发 `agent_config`（同时取 `context_window` 与 `session_id`），provider request id → 每响应一条 `provider_request`（旧实现挂 `message_end` 只能留最后一个）；钩子只留 `onEvent` 分发；sessionId 改由适配层生成（sink 只绑 key/role，`append` 时与事件自带 agent 合并）。新增 `packages/trajectory/src/session.ts`（纯解析，无 node 依赖）+ `replayRun({events, sessions})`（无会话回退 `replayRunEvents`，旧 run 可读）+ `summarizeRunEvents` → `RunDigest`；`packages/utils` 新增 `readSessionFacts`（并新增 `@zread-pi/trajectory` 工作区依赖，无循环）。浏览站新增 `GET /api/runs/:runId/sessions`，前端 `useTrajectoryLayout` 第 4 参数 `sessions` 走 `replayRun`。**取舍**：浏览站轨迹页运行中不再有 partial 流式预览（内容在消息结算后才落盘；TUI 生成页进度不受影响，走 `onEvent`）
...
| 图片处理管线（`image/`） | 新增 | 移植自 pi coding-agent 的 `utils/image-*.ts`（7 个文件）：`processImage()` 是唯一入口；Worker 线程跑 photon（Rust/WASM），失败回退进程内；BMP/TIFF 等转 PNG；EXIF Orientation 真正旋转像素。**与上游的差异**：去掉「Bun 下先试相对仓库源码路径」分支（cwd 是目标仓库，该路径无意义）；仅在模型支持图片时才处理，失败降级为文本说明（`[Image omitted: …]`，不报 tool error）。**依赖与打包**：`@silvia-odwyer/photon-node` 0.3.4（三平台均有 wasm，无原生编译）；tsup `onSuccess` 把 `photon_rs_bg.wasm` 复制进 `apps/cli/dist/`（按 `__dirname` 读取），`tools/build-binary.ts` 复制到 standalone 二进制旁（按 `process.execPath` 同目录查找），缺失时图片退化为文本说明 |
| `submit_sections` / `submit_section_topics` / `refine_section_titles` | **新增（三阶段蓝图输出工具）** | 分类阶段写骨架 / sync 合并分类；主题阶段按分类增量归并页面（slug 由代码分配）；标题阶段批量写回 title。单 section 失败不阻断（见 §1.2）；`generate_blueprint` / `generate_sync_blueprint` 保留仅归档 |
...
`NodeExecutionEnv` 与 `Context`，排队逻辑仍在 pi 里）。
...
| 命令                 | 覆盖内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 期望                                      |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| `test:catalog`     | pi-ai Provider 目录、api_key 登录写 auth.json、多 Provider、自定义模型、未内置 Provider（**`llm.providers.<id>.name` 往返保留 / 列表用配置名 / 缺省回退 id**）、runtime model、思考深度支持列表、**自定义模型 `thinking_level_map` 透传 + pi opt-in 语义（声明 max 则 max 生效 / xhigh 向上钳到 max / 未声明仍是普通推理模型）+ `validateConfig` 三态保留与非法键值过滤**、旧配置补 `agent.max_turns` 默认值（`0` 保留为不限制、负数回退 30）、`blueprint.detail` 缺省 high / 合法值保留 / 非法值回退 high、`llm.context_window` / `llm.max_tokens` 缺省 null / 合法值保留 / 0 与负数回退 null、logout、**`quality` 归一化（缺省 enabled=true + mode=warn + verifyAfterGenerate=false / 合法值保留 / 非法值回退）**                                                                                                                                                                                                                                                                                                                         | 57/57                                   |
| `test:agent`       | pi 循环、工具执行、钩子、流式事件、429 重试、usage、thinkingLevel 透传、maxTurns、Provider 重试配置透传（`streamOptions`）、`toRetryPolicy`/`toStreamOptions` 纯函数映射（`maxRetries=0` 显式禁用）、`TokenUsage` 归并（`addTokenUsage`/`sumTokenUsage`，缺失缓存字段按 0；`system/init` 带 `context_window`）                                                                                                                                                                                                                                                                                                                       | 20/20 + 12/12 + 12/12 |
...
| `test:blueprint`   | Orchestrator 三阶段蓝图端到端：`generateWikiCatalog()` 分类/主题/标题三阶段落盘 `wiki.json`（骨架即 pages 为空也可加载、slug/file 由代码分配、标题写回）；**蓝图细节档位（`test/blueprint-detail.ts`）：五档规格与提示词注入、常驻数量反馈、越界不落盘 + 2 轮后缩编 subagent 成功 / 失败降级代码兜底（日志含兜底注记）、minimal 单分类单篇 / 同步 merge 上限语义 / 档位目录落盘与遗留兼容 / listWikiVariants 与 resolveWikiVariant**；**两个基础分类（概览/核心架构）：空输入只强补 2 个、模型主动提交「快速开始」仍作普通分类保留、该角色页面不再强制 Mermaid**；**scope / summary 逐级注入：字段透传 + 旧数据兼容 + 提示词与工具 schema + 全链路落盘（B9 / A10 / C1 / C2）**；跨 Agent 聚合用量 = 所有请求之和；**逐 Agent 行（每个 Agent 一行）：`agentKey`/`agentRole`/`agentStatus`、`agentUsage`、上下文窗口 200k、失败分类行标 failed**；**`llm.context_window` / `llm.max_tokens` 覆盖下发：`system/init` 上报覆盖后的窗口 + 请求输出上限为覆盖值**；模型不产出 sections / 某分类不调工具必须报错或记 `failedSections`（不阻断其余）；增量同步 diff（updated/archived/new/unchanged、URL 不漂移、无变更不调 LLM、漏报兜底）；目标仓库/全局 `AGENTS.md` 注入系统提示（`<project_context>`，全局在前）+ **文风纪律注入（`<writing_discipline>` 排在 project_context 之后）** + 上下文文件加载纯函数（候选优先级 / BOM / 大小写变体 / 截断 / 格式）+ 文风纪律纯函数（语言选择 / 注入块 / polish 提示词 / 保护性约束 / vendored 行数）；**捕获点端到端（场景 8）：runLog 自动落盘 + 会话 join 回归**——不传 runLog 时自动建 run、run.json 记 kind/detail/model、events.jsonl 首尾为 run_start/run_end 且 seq 从 1 单调、`agent_config` 全部携带**互不相同**的 sessionId 且与 `<runDir>/sessions/` 的会话文件一一对应（7 / 7）、**内容类事件不再出现在 events.jsonl**、`provider_request` 每响应一条且互不相同、用真实捕获事件流 + 会话文件跑 `replayRun` 断言内容按 sessionId 归入各 turn（mismatch=0，max_concurrent=4 交错）                                                                                                                                                                                                                                                                                                                     | 59/59 + 116/116 + 23/23 + 11/11 + 17/17            |
...
| `test:logger`      | 日志总线（对齐 cordis）：记录形状（sn/ts/name/type/level）、printf 全占位符 + Error/cause/AggregateError 展开 + 10240 截断、**`Logger.code` 哈希与 harness 逐字一致（黄金值取自 vendor/cordis 实跑）**、按 exporter/按名级别阈值（含 `ZREAD_PI_LOG_LEVEL` 解析与非法值回退）、多 exporter 广播 + 1000 条环形缓冲淘汰、file exporter 行格式 / 跨天路径 / 保留期清理（含负数禁用）/ needle 兼容、**JSONL exporter（默认开启 / `ZREAD_PI_LOG_JSONL=0` 关闭，字段完整 / printf 同语义 / sn 单调）**、**文本 file-exporter 默认关闭（`ZREAD_PI_LOG_TEXT=1` 开启）与 needle 显式开启**、**递归保护（`tui.stdout` / `tui.console` 不回 console）**、**无参调用与故障 exporter 的异常隔离**、命名 logger 端到端、`getLogFile()` 稳定性、console 渲染（无色 / 16 色 / 256 色 / showDiff / label 宽度）与色彩探测、`Time` 工具 | 115/115（+1 源码守卫：业务层 logger 不得打印模型内容）|
| `test:pages`       | 并行页面生成：`generateWikiContent()` + `write_page` + Mermaid 校验；页面未落盘（未调用 `write_page` / 写入路径不符 / Mermaid 拦截）必须记失败并发出 `page_error`；预算耗尽后仍无产物同样计页失败（工具熔断 → 强制交卷 → 仍无 `write_page`）；**失败事件带最后一次累计用量**（`page_error.usage`，底部合计的依据）；**页面级 polish（`polish.mode=full`）：真实 Edit 生效、Mermaid 改坏回滚、no-change / Agent 失败都不判页失败、prompt-only 与 enabled=false 的开关语义**；**minimal 档位页面提示词附加「全景导览」段（Mermaid 架构图）**；**topicSummary 注入 / 缺省省略 / 范围纪律（B9 页面提示词）**；**页面事件带上下文报表值（完成 / 失败都带）**；**内容密度门（`content-gate.ts`）：度量计算 / 下限表 / enforce 拦截 + 「当前 N / 下限 M」反馈 / warn 只报告 / 反注水（代码块是软建议）/ 配置归一化 / `OVERVIEW_SECTIONS` 只含概览与核心架构（快速开始角色已移除，不再强制 Mermaid）**；**enforce 降级落盘（`page-gate-degraded.ts`）：预算耗尽后 best-effort 落盘 + gate 注释 + 计成功 + `mode='enforce-degraded'`**；**`checkPolishDiff` 纯函数（8 项）：只改散文安全 / 改 frontmatter / 改 Sources 行号 / 删 Sources 行 / 改 Mermaid 节点 / 改块外散文安全 / 多项越界全部列出 / 完全相同** | 24/24 + 7/7 + 16/16 + 76/76 + 8/8 + 24                     |
| `test:verify`      | 交付闸门（`verify-wiki.ts`）：`parseSourceRefs` 纯函数（路径 / `#Lx-Ly` / `#Lx` / 外链与纯锚点排除）；五组全绿（structure / content / mermaid / frontmatter / traceability）；structure 三类失败（文件缺失 / `(section,file)` 重复 / sections 集合不一致）+ **`associatedFiles` 存在性**；content 的 `--enforce` 与非 enforce 语义（只列出不记 FAIL）；mermaid 节点标签失败；frontmatter 缺失与不一致；traceability 路径不存在 / 行号越界（含文件总行数）/ 无 Sources FAIL / 跨页重复声明只 WARN / **符号缓存缺失 SKIP**；遗留目录回退（`legacy=true`）；无产物整体 SKIP（ok=true）；骨架 pages 为空 → 其余组 SKIP；档位自动解析 / 显式 / 不存在回退；**`verifyAfterGenerate` → `<runId>/verify.json` 落盘且 `run.json` 契约不变** | 54/54                                   |
| `test:page-format` | 页面格式契约 + reader-first 提示词资产层：zh/en 两份资产节数（6）/ 列表项 / 清单条数（11）一一对应；**§4 反注水三条在双语都存在**（同义改写注水 / 不复制 README·AGENTS.md / 「源没有就应该是 0」）；语言选择与回退（`en` 之外一律中文）；注入块标签与开关（`withPageFormat` 始终注入，`withReaderDiscipline` 受 `enabled` 控制）；reader-first 拼在 humanizer 之后；`buildPagePrompt` 含格式契约且保留既有段落（`en` 选英文契约、块只注入一次、minimal 仍附加全景导览）；polish 系统提示 = 纪律 + 读者自检 + Embedded mode 且顺序正确 | 40/40                                   |
| `test:golden-parity` | §5.5 一致性校验（强制项）：用**同一输入**在 lecture-to-notes 的 Python 参考实现（`verify_notes.CJK` / `extract_claims.numbers_in`）与 TS 实现上跑黄金值对照——6 个固定样本的 CJK 计数必须逐位一致（样本与 `tools/golden-parity-gen.py` 逐字同步，黄金值由该脚本生成，可复现）；数字台账口径（千分位 / 小数点续接为一段）；区间端点边界语义（U+4E00 / U+9FFF 计入，全角空格·逗号不计入，空串 = 0） | 15/15                                   |
| `test:titles`      | 标题诊断信号 + `refine_section_titles` 自检：提示词含 7 条诊断信号（子层级编号 / 续接词 / 相邻同母题 / 字数悬殊 1/5 / 具体技术点 / 母题词反复 / 命名风格）与收敛优先策略；**数量一致性自检：陌生 slug / 漏 slug → `is_error` 且不落盘**（标题未变）；正确提交 → 落盘 + slug/file/section/associatedFiles 逐字不变 + `onResult` 带 updated=2/skipped=1/unknown=0；`expectedSlugs` 缺省时跳过数量自检（旧调用点兼容） | 28/28                                   |
| `test:traceability` | 溯源台账纯函数（`traceability.ts`）：`collectKnownSymbols`（exports / functions / imports；空清单 size=0）；`findUnresolvedSymbols`（幻觉识别 / **围栏代码块剥离** / 过短名与非标识符噪声过滤 / 上限保护 / known 为空时跳过）；`collectManifestPaths`（反斜杠归一化）与 `isPathReal`（manifest 命中或磁盘存在）；`countLines`（多行 / 空文件 / 不存在 → undefined）；`checkAssociatedFiles`（缺失列出 / 无声明不报）；`checkTraceability` 端到端（badPaths / badLines 带总行数 / **反转区间 L3-1 判失败** / 单行 `#L2` → 2..2 / 跨页重复 / noSources / 符号维度 / 外链锚点排除） | 37/37                                   |
| `test:browse`      | 「浏览文档」服务器 + pi-tui 浏览页：静态资源/API 同端口、SPA fallback、未知 API 404、`close()` 后可连性；页面显示真实地址、ESC 停止；源码无产物时进程内 Vite 兜底；无效资源目录报错；**多档共存：`/api/wiki/variants`、`?detail=`（catalog/content/source）、非法值 / 缺失档位 404、遗留回退、变体目录正文解析、仅有变体的目录识别**；**界面语言 `GET /api/i18n`：`zh→zh-CN` / `en→en-US` / 改配置后按请求即时生效**                                                                                                                                                                                                                                                                                                                                                      | 78/90（无 `apps/browse/dist` 时 +4 项 Vite 兜底） + 组件 51 |
...
| `test:tui`         | `smoke-tui.ts`（布局/按键/输入框/长列表分页/终端自适应/按键重绘与 Kitty 松开过滤/Provider 详情页 API Key+模型焦点切换/多 Provider/自定义模型/思考深度页/模型上下文输出覆盖页（字段切换 + 留空跟随默认 + d 恢复默认 + 非法输入拦截 + 写回与落盘）/最大轮次页（含 `0` = 不限制写回与落盘）/**文风润色页（开关切换 + prompt-only/full 写回与落盘）**/**蓝图细节档位页（五档切换 + 写回与落盘）**/**内容质量门页（模式切换 + t 开关 + v 自动校验 + s 保存落盘）**/外部工具页/版本号与项目版本同步 **+ 自定义 Provider 新建全链路（名称/URL/协议步骤 + 空名称与非法 URL 拦截 + `t` 切换协议 + 创建后进入详情页 + 连续添加两个模型 + `e` 编辑预填与改名 + 列表回显）+ id 生成纯函数（slug / 中文 / 回退 / 冲突追加序号）+ 自定义模型的 pi 扩展思考档（开启思考后出现 xhigh/max 开关 + 写入 `thinking_level_map` + 详情页 max 徽标）** 313 项）、`render-all-routes.ts`（全部 24 个路由渲染不报错、无超宽行）、`real-run-check.ts`（真实 ProcessTerminal 启动/退出 9 项）、`output-guard.ts`（stdout 接管：杂散重定向 / 放行窗口 / ESC 透传 / 日志 / 还原 10 项）、`cli-target-dir.ts`（`-d/--dir`：绝对/相对路径、`wiki --dir` 写法、产物落盘到目标目录、调用目录不被写入、缺省行为、无效目录报错 36 项）、`bun test src/views/wiki-generate/__tests__`（事件映射 + 用量合计 + 槽位结转 + 三阶段进度映射 + 逐 Agent 行 + 42 项：失败事件保留快照 / 重试不清零 / 幂等 reduce / 0 分母边界 / 生成中忽略重复触发 / stage+section+分类级进度 / scanning 清空 / plan→running→completed 行状态 / `agentUsage` 不是聚合 / 整体 complete 保留行 / 页面上下文沿用与清零 / `contextUsage` 边界 / **内容门报告映射（warn / enforce-degraded / 失败页携带 / 重新生成清零）**）、`mock-generate.ts`（生成 + 同步全链路 + 全局记忆写入 + **底部用量合计与重试累加** + **三阶段进度文案** + **逐 Agent 行与完成后的四指标**断言 46 项）、`browse-server.ts`（浏览文档服务 + 页面 + 多档变体 API，50 项：无 `apps/browse/dist` 时含 4 项 Vite 兜底） | 313 + 9 + 10 + 33 + 42 + 43 + 50 + 51（组件）             |
| `test:trajectory`  | 轨迹模型层（106 项）+ **会话投影层（78 项）**：replay 折叠（turn=agent / group 归属 / 请求统一编号 / 失败归属 / partial）、**并发归属（session id：交错事件流不互相吞 / agent_end 不清空别的 Agent 的在途消息 / 旧日志无 sessionId 回退 key）**、**turn 的 sessionId 唯一性**、layout、timeline 四模式投影 + 选区过滤、**session 隐藏后时间线重新投影**、搜索索引（多词交集 / 增量更新）、虚拟化窗口数学 + 稳定 key、格式化函数、**大规模布局回归（10 万 cell：partial 追加不抛错 / 续号 = 最大索引 + 1 / 时间线 span 数与范围 / 显式 lastIndex 向后兼容）**；store 层（47 项）：writer/reader 往返、beforeSeq/afterSeq 分页、损坏行跳过、保留期清理（删最早开始的 run）、残留 running 自愈 interrupted、run.json 写入串行（end 终态不被挂起写覆盖）、withRunLog 自动建 run / 复用 / 失败记 failed、runId 校验与列表排序 | 106/106 + 78/78 + 53/53 + 31/31 + 7/7                    |
...
| 适配层 `packages/agent-runtime/**`                                                                     | `bun run test`（全部套件）+ 新增/更新针对性断言                                   | 契约面改动必须同步本文件 §1.2 契约冻结点；首尾机制（预算/提示/终止）改动必须补 `test:context` 断言 |
...
| 标题精修（`prompts/titles.ts` 诊断段、`tools/output-tools.ts` 的 `createRefineSectionTitlesTool`、`agents/blueprint-stages.ts` 的标题阶段、`utils/output/wiki-content.ts` 的 `applySectionTitles` 注释） | `bun run typecheck` + `bun run test:blueprint` + `bun run mock:wiki` + `bun run test` | 诊断信号改动必须同步 `titles-diagnostic.ts` 的 7 条断言；`expectedSlugs` 自检必须**在落盘前**执行（失败 = is_error + 不落盘）；`expectedSlugs` / `onResult` 必须保持**可选**（旧调用点兼容）；提示词的工具调用指令与既有 8 条输出规范不得动 |
...
| 基础分类 / 蓝图骨架（`utils/src/output/wiki-content.ts` 的 `BASE_SECTIONS`、`orchestrator/src/agents/blueprint-detail.ts` 的 `BLUEPRINT_DETAIL_SPECS` + `BASE_SECTION_NOTE`、`prompts/classify.ts` 的数量目标与示例、`tools/output-tools.ts` 的 `submit_sections` 文案、`wiki/content-gate.ts` 的 `OVERVIEW_SECTIONS`） | `bun run typecheck` + `bun run test:blueprint` + `bun run test:pages` + `bun run test:tui` + `bun run mock:wiki` + `bun run test` | 增删基础分类必须同步：① sections 区间下限（`judgeQuantity` 判的是归一化后的 count，N 个基础分类则下限随 N）；② `OVERVIEW_SECTIONS` 的 Mermaid 强制角色；③ 测试夹具里所有**靠「基础分类 + 领域」凑越界**的清单（如 `OVER_SECTIONS`，少一个基础分类要补一个领域才能继续触发越界路径，否则缩编链路**静默失效**）；④ mock 类夹具的页数与用量合计算术（`formatBytes` <1000 不带 k）；⑤ §1.1 档位决策表与 `README.md` 的区间描述 |
| 交付闸门（`orchestrator/src/wiki/verify-wiki.ts`、`apps/cli/src/commands/verify.ts`、`generate-wiki.ts` 的 `verifyAfterGenerate` 集成、`cli/index.ts` 的 `verify` 子命令） | `bun run typecheck` + `bun run test:verify` + `bun run test`（i18n / CLI 注册改动另跑 `bun run test:tui`） | 检查组 / PASS-FAIL-SKIP 语义 / 退出码改动必须补 `test:verify` 断言；**不得在 verify-wiki.ts 里写任何产物**（只读；`verify.json` 由 CLI / generate-wiki 落盘）；`RunMeta` 不得改动（摘要走独立 `verify.json`）；`verifyAfterGenerate` 必须缺省 false |
| 溯源台账（`orchestrator/src/wiki/traceability.ts`、`verify-wiki.ts` 的 traceability / structure 检查组、`tools/output-tools.ts` 的 `ValidateBlueprintTool` 归档注释） | `bun run typecheck` + `bun run test:traceability` + `bun run test:verify` + `bun run test` | 符号 / 路径 / 行号 / 重复声明语义改动必须补 `test:traceability`（纯函数）与 `test:verify`（检查组集成）；**不得在 traceability.ts 里写任何产物**（只读）；符号层必须保持 WARN（不 FAIL），蓝图 `associatedFiles` 必须是 FAIL；`parseSourceRefs` 迁移后旧导入路径必须保留（verify-wiki re-export） |
| 内容密度门（`orchestrator/src/wiki/content-gate.ts`、`tools/page-tools.ts` 的门拦截、`wiki/generate-wiki.ts` 的降级落盘、`wiki/types.ts` 的 `PageResult.gate`、`types/config.ts` 的 `QualityConfig`、`utils/src/config` 的 `normalizeQualityConfig`、`views/config-quality/**`、CLI 生成页的 ⚠ 标记、`quality.*` 配置） | `bun run typecheck` + `bun run test:pages` + `bun run test:catalog` + `bun run test:tui` + `bun run test` | 门限 / 度量 / 拦截 / 降级语义改动必须补 `test:pages`（`content-gate.ts` 纯逻辑 + `page-gate-degraded.ts` 端到端）；配置归一化改动补 `test:catalog`；配置界面改动补 `test:tui`（`smoke-tui` 用例 4i）；**降级落盘必须同时覆盖 `try` 与 `catch` 两条路径**（预算耗尽走 `catch`）；工具名 / schema / 提示文本不要改 |
...
| 会话投影（方案 C：`packages/trajectory/src/session.ts`、`replay.ts` 的 `replayRun` / `summarizeRunEvents`、`utils/src/trajectory-store/session-reader.ts`、`agents/create-agent.ts` 的捕获层、`agents/run-log-sink.ts`、`run-log-writer.ts` 的 builder、`browse-server.ts` 的 `/api/runs/:runId/sessions`、`useTrajectoryEvents` / `useTrajectoryLayout` 的 sessions 参数） | `bun run typecheck` + `bun run test:trajectory` + `bun run test:blueprint` + `bun run test:browse` + `bun run mock:wiki` + `bun run test` | 会话解析 / join / 用量口径改动必须补 `session-replay.ts`（纯逻辑）与 `e2e-blueprint` 场景 8（端到端会话 join）；新增事件 kind 必须同步 `ActiveRunEventKind` 与 `run-log-writer` 的 builder，**内容类 kind 不得复活**（内容只在会话里）；`packages/trajectory/src/session.ts` 必须保持**无 node 依赖**（被 Vite 打包）；捕获层改动必须跑 `mock:wiki` 核对「会话文件数 = agent_config 数」与 events.jsonl 只剩瘦事件 |
...
| 页面格式资产 / reader-first 纪律 / polish 复检（`prompts/page-format.*.md`、`prompts/reader-first.*.md`、`agents/page-format.ts`、`agents/reader-first.ts`、`prompts/page-agent.ts` 的格式段抽取、`generate-wiki.ts` 的 `buildPagePrompt`、`create-agent.ts` 的系统提示拼装、`style-discipline.ts` 的 `buildPolishSystemPrompt`、`wiki/polish.ts` 的 `checkPolishDiff` 与回滚） | `bun run typecheck` + `bun run test:page-format` + `bun run test:pages` + `bun run mock:wiki` + `bun run test` | 两份 `.md` 资产**必须同步改动，条数与编号一一对应**（zh / en）；**§4 反注水三条与「源没有就应该是 0」必须在双语都存在**；`page-agent.ts` 只做文本外置，**叙述 / 语气 / 结构要求逐字保留**；改资产后跑 `mock:wiki` 确认生成链路不破（`overall=PASS`）；回滚保护扩展必须覆盖 frontmatter / `Sources:` / Mermaid 三项 diff 断言；工具调用指令（`write_page` 参数、输出路径规范）不得动 |
| 一致性校验 / 黄金值对照（`tools/golden-parity-gen.py`、`packages/orchestrator/test/golden-parity.ts`、`content-gate.ts` 的 `countCjkChars` / `numbersIn`） | `python3 tools/golden-parity-gen.py` 重新生成 + `bun run typecheck` + `bun run test:golden-parity` + `bun run test` | 样本变动必须**两边同步**（Python 脚本与 TS 测试逐字一致）并重新生成黄金值；判定语义变动须在本文件 §1.1 声明为有意偏差；带 `g` 标志的正则不得用于逐元素 `test()`（`lastIndex` 状态会跳过元素） |
...
| 重试策略（`agent-runtime/src/retry.ts`、`harness/driver.ts`、orchestrator 的 `create-agent.ts`）           | `bun run typecheck` + `bun run test:agent` + `bun run test:agent:http` + `bun run test` | `RetryConfig` 形状改动必须同步本文件 §1.2 的 `RetryConfig` 契约；两层重试的映射/禁用语义必须补 `test:agent`；`Retry-After` 行为必须补 `test:agent:http`          |
...
- [ ] 改动范围与需求一致，没有顺手改无关文件
- [ ] 没有破坏 §1.2 的契约冻结点（若必须改，业务层同步修改 + 本文件 §1.2 更新）
- [ ] `bun run typecheck` 0 错误
- [ ] 与改动类型匹配的测试全绿（§3），且输出被真实记录
- [ ] 新增/变更的行为有对应断言（不留"只改实现不补测试"的改动）
- [ ] 文档同步：`README.md`（命令/用法）、本文件 §1.1/§1.2（决策/契约/行为差异）
...
- `vendor/pi/packages/agent/package.json` 的 `exports` 除上游原有的入口外，额外露出 3 个**内核实现子路径**（仅 manifest 改动，源码零改动）：
  ...
  本仓库不引入 `pi-coding-agent`，因此用子路径导出直接消费同一份构建产物。
...
- **本仓库对 vendor 源码的唯一改动**（v1.21.0，两处，均在 `agent` 包，带 `// zread-pi:` 注释）：
  1. `harness/env/nodejs.ts` 的 `listDir`：单条目 `lstat` 抛 ENOENT 时**跳过该条目**而不是让整个目录列举失败。原因：并发会话创建会写瞬态 `<file>.tmp` 再 rename（`session/jsonl/io.ts`），该文件可能在另一路 `create` 的 `readdir` 与 `lstat` 之间消失，原实现会让 `create` 抛 `Failed to list sessions directory` → 分类 Agent 失败（`test:blueprint` 的 C1 场景约 2/7 概率复现）。
  2. `harness/env/nodejs.ts` 新增 `isENOENT` 辅助函数（复用既有 `isNodeError`）。
  改完必须跑 `bun run vendor:build`（tsconfig 顺序 telemetry → chord → ai → agent → tui）；回归守卫在 `test:agent` 的 `session-persist.ts`（并发 create + 列举途中删除，禁用修复时 30/30 轮失败，启用后全绿）。
...
- 产物布局（多档共存）：`<目标仓库>/.zread-pi/wiki/<detail>/wiki.json + <detail>/<section>/<file>.md`（每个档位一套完整产物）；遗留的无档位 `wiki/wiki.json` 只读兼容，browse 中显示为「默认」。路径唯一口径是 `getWikiDir(detail?)` / `getWikiJsonPath(detail?)` / `listWikiVariants()`（见 §1.1）。
...
- **`Bash` / `PowerShell` 等 shell 执行工具未迁移**（第六步只做「文件与搜索」工具）：因此工具文案一律不引用 shell
  ...
  （沙箱/审批/超时/输出截断/Windows 分支）。
- **五个阶段已全部落地，且已做过一轮逐条复执行**（v1.18.1）：
  内容密度门（v1.14.0）、verify-wiki 交付闸门（v1.15.0）、溯源台账（v1.16.0）、
  页面格式资产 + reader-first（v1.17.0）、标题诊断信号（v1.18.0）。
  复执行补齐了三项：反注水清单（同义改写注水 / 不复制 README·AGENTS.md /
  「源里没有代码就应该是 0」）、黄金值对照（CJK 计数与数字台账口径与
  lecture-to-notes 的 Python 参考实现逐位一致）、mock 生成后自动跑 verify 闸门。
  这些改动的**真机长期使用**
  （真实 API Key + 真实仓库）尚未进行——全部验证基于自建夹具 / mock LLM /
  合成缓存。
- **「快速开始」强制基础分类已移除**（v1.19.0，五阶段之外的取向修正）：
  非 minimal 档位只剩概览 / 核心架构两个基础分类；`low` 区间 3~5 → 2~5、
  medium/high/max 下限 4 → 3；该角色的页面不再强制 Mermaid。理由与两个执行期
  踩到的硬伤：受众错配（安装步骤是 README 里 10 秒能拿到的东西，不是需要 Agent 读源码综合的内容）、可信度风险最高（工具不执行安装命令，最容易写出「看起来对、跑不通」的命令与版本号，而溯源机制对安装步骤最无力）、与反注水机制自相矛盾（`codeRecommendedFor` 要求代码块来自关联源文件，但该页天然该写 shell 命令，同时又被强制要求 Mermaid，只能注水凑）、一刀切（内部微服务 / 纯模块库没有外部安装故事）。wiki 由此不再提供「怎么跑起来」的入口，
  **有意为之**（定位是源码理解，安装说明归 README）。
...
- **会话已 durable，但「中断续跑」未接线**（v1.21.0 起）：方案 C 之后 pi 会话落
  `<runDir>/sessions/--<cwd>--/<ts>_<sessionId>.jsonl`（`JsonlSessionRepo`，
  完整内容、可 fork），但编排层仍然**每次 `query()` 新建一个会话**——driver
  只 `repo.create()`，从不 `repo.open()`。要启用续跑需要：① 适配层
  `createAgent({ sessionId })` 显式传入既有 id；② driver 按 id `open` 而非
  `create`；③ 编排层从同一 run 的 `agent_config` 事件里按 agent key 查回
  上次的 sessionId。旧 `saveSession/loadSession/tag/fork` 仍未迁移
  （pi 侧是 JSONL 会话树 + SQLite）。
...
- **时间线的「缩放达到阈值后自动切 actual 模式」未实现**：`MINIMUM_ZOOM_OPERATIONS`
  ...
  duration 已实现（原生非被动监听，缩放不再连带滚动台账）。
- **轨迹视图的大规模真机渲染未做人工实测**：sticky 表头 / 滚轮缩放 / sub-pixel span 合并 /
  流式期间的缩放保留与搜索节流均已改过并通过构建与模型回归，
  且渲染 / 交互契约（sticky 表头位置、span 合并、滚轮模式切换）已有**组件级测试**
  兜底（`test:components`，51 项，见 `apps/browse/test/README.md`）；
  但真机（上万吨记录、运行中的 run、高分辨率屏）下的帧率与交互仍需人工确认。
- **浏览站轨迹页在运行中不再有 partial 流式预览**（方案 C，v1.21.0）：
  partial 帧语义来自内容事件，内容改为只在 pi 会话里、消息**结算后**才落盘，
  所以运行中的 run 在轨迹页上是「逐条消息出现」而不是「看着正文长出来」。
  TUI 生成页的实时进度（token 计数 / 页面状态 / 逐 Agent 行）不受影响——
  那条链路走 `onEvent`，不依赖轨迹事件。若真机使用证明需要实时预览，
  可改为尾随解析 append-only 的会话文件，不用回到双写。
...
5. 工具层可选补强（未决）：`Grep` 的 `type` 参数、rg/fd 自动下载、
   以及「先用方案再实现」的 shell 执行能力。（`Read` 图片缩放已由第十二步的图片处理管线完成，见 §1.1）
