...
| `test:page-format` | 页面格式契约 + reader-first 提示词资产层：zh/en 两份资产节数 / 列表项 / 清单条数一一对应；语言选择与回退（`en` 之外一律中文）；注入块标签与开关（`withPageFormat` 始终注入，`withReaderDiscipline` 受 `enabled` 控制）；reader-first 拼在 humanizer 之后；`buildPagePrompt` 含格式契约且保留既有段落（`en` 选英文契约、块只注入一次、minimal 仍附加全景导览）；polish 系统提示 = 纪律 + 读者自检 + Embedded mode 且顺序正确 | 37/37                                   |
...
- **plan.md 的五个阶段已全部落地**：内容密度门（v1.14.0）、verify-wiki 交付闸门（v1.15.0）、
  溯源台账（v1.16.0）、页面格式资产 + reader-first（v1.17.0）、标题诊断信号（v1.18.0）。
  见 `MIGRATION.md` §29–§33。这些改动的**真机长期使用**（真实 API Key + 真实仓库）
  尚未进行——全部验证基于自建夹具 / mock LLM / 合成缓存。
