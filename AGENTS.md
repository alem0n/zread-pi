...
| `test:verify`      | 交付闸门（`verify-wiki.ts`）：`parseSourceRefs` 纯函数（路径 / `#Lx-Ly` / `#Lx` / 外链与纯锚点排除）；五组全绿（structure / content / mermaid / frontmatter / traceability）；structure 三类失败（文件缺失 / `(section,file)` 重复 / sections 集合不一致）；content 的 `--enforce` 与非 enforce 语义（只列出不记 FAIL）；mermaid 节点标签失败；frontmatter 缺失与不一致；traceability 路径不存在 / 行号越界（含文件总行数）/ 无 Sources FAIL / 跨页重复声明只 WARN；遗留目录回退（`legacy=true`）；无产物整体 SKIP（ok=true）；骨架 pages 为空 → 其余组 SKIP；档位自动解析 / 显式 / 不存在回退；**`verifyAfterGenerate` → `<runId>/verify.json` 落盘且 `run.json` 契约不变** | 54/54                                   |
...
- **内容密度门已落地，交付闸门 `verify-wiki` 已实现但未在真机长期使用**：
  `zread-pi verify` 命令与 `quality.verifyAfterGenerate` 自动校验均已落地
  （`MIGRATION.md` §30），但全部验证基于自建夹具与 mock:wiki 产物；
  真实仓库（多档位共存 / 遗留目录 / 大量页面）下的性能与判定准确性仍需人工确认。
  溯源台账的符号级校验（P1-1）、页面格式资产化（P1-2）、标题诊断信号（P2）
  亦未开始，见 `plan.md`。
