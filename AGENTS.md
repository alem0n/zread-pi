...
- **溯源台账已落地（符号层 + 蓝图声明）**：`traceability.ts` 纯函数 + 只读，
  复用 repo-analyzer 已产出的缓存清单（`last_symbols.json`）零额外解析成本
  （`MIGRATION.md` §31）。**符号层只 WARN 不 FAIL**；**蓝图 `associatedFiles` 不存在直接 FAIL**。
- **页面格式资产化 + reader-first 纪律已落地**：`page-format.*.md`（硬性格式契约 +
  8 条交付前自检清单）与 `reader-first.*.md`（教学型写作纪律 8 节）两份资产已抽取并接入
  页面提示词与 Agent 系统提示（`MIGRATION.md` §32）；polish 的回滚保护已升级为
  「只许改散文」的三项 diff 断言。剩余未做：**标题诊断信号（P2）**，见 `plan.md` §3.5。
