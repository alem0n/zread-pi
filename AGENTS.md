...
| 页面内容密度门（`quality.contentGate`，v1.14.0） | 迁移自 lecture-to-notes 的 `density_gate`（详见 `plan.md` / `MIGRATION.md` §29）：把「这篇页面是不是干瘪 TL;DR」变成机械指标（散文字符 / 标题层级 / Mermaid / 代码块 / Sources / 重复句首），在 `write_page` 的 Mermaid 校验之后拦截。`enforce` 未达标返回 `is_error` + 「当前 N / 下限 M」常驻反馈让模型重写；预算用尽后 `writeDegradedPage()` best-effort 落盘（`gate.mode='enforce-degraded'`，**计成功 + 告警**，不判页失败——「生成永不悬挂」）；`warn`（缺省）只报告不拦截。反注水：门限是下限不是目标、代码块是软建议（源里没代码时 0 是正确答案）。配置界面 `/config/quality` |
...
| 重试分两层，尊重服务端 `Retry-After`（第十二步）             |
| 方案 C：pi 会话 = 唯一完整事实源（第三十步） | 轨迹内容双写（pi 会话 + 捕获层再抄一遍）有三个硬伤：截断失真（预览 512 / 输出 64KiB / 摘要 8KiB）、重复劳动（流式状态机 / 工具错误暂存 / request id 挂终态时序协议）、口径漂移（两份拷贝各自演进）。阶段 0–1 已让会话可读（探针 + `sessionRoot` → `JsonlSessionRepo` 落 `<runDir>/sessions/`）；本步做消费侧：内容全部从会话投影，`events.jsonl` 只留瘦业务事件（`agent_config` + `agent_end` + 每响应一条 `provider_request` + run/page/stage/section/scan）。`agent_config` 承载三个不在会话里的 harness 配置事实（systemPrompt 全文 / toolCatalog schema / tokenBudget）+ 会话 id；失败归属用 `keyToIdentity` 双键回退（sink 绑定只有 key）。旧 run 无会话目录 → 回退 `replayRunEvents`。**取舍**：浏览站轨迹页运行中无 partial 预览（TUI 生成页进度不受影响）。详见 `MIGRATION.md` §36 | Agent 层（harness `RetryPolicy`）从固定 10s 改为指数退避 2s→4s→…、60s 封顶；Provider 层新增 `streamOptions.maxRetries / maxRetryDelayMs`，由 pi-ai `retryProviderRequest` 读服务端 `Retry-After`，超上限立即失败并交给 Agent 层。`maxRetries=0` 现在**显式禁用**（不再回退 harness 默认 3 次） |
...
| 蓝图细节档位（`orchestrator/src/agents/blueprint-detail.ts`、`tools/output-tools.ts` 数量控制、`prompts/classify.ts`、`prompts/topics.ts`、`views/config-detail/**`、`blueprint.*` 配置） |
| 基础分类 / 蓝图骨架（`utils/src/output/wiki-content.ts` 的 `BASE_SECTIONS`、`orchestrator/src/agents/blueprint-detail.ts` 的 `BLUEPRINT_DETAIL_SPECS` + `BASE_SECTION_NOTE`、`prompts/classify.ts` 的数量目标与示例、`tools/output-tools.ts` 的 `submit_sections` 文案、`wiki/content-gate.ts` 的 `OVERVIEW_SECTIONS`） | `bun run typecheck` + `bun run test:blueprint` + `bun run test:pages` + `bun run test:tui` + `bun run mock:wiki` + `bun run test` | 增删基础分类必须同步：① sections 区间下限（`judgeQuantity` 判的是归一化后的 count，N 个基础分类则下限随 N）；② `OVERVIEW_SECTIONS` 的 Mermaid 强制角色；③ 测试夹具里所有**靠「基础分类 + 领域」凑越界**的清单（如 `OVER_SECTIONS`，少一个基础分类要补一个领域才能继续触发越界路径，否则缩编链路**静默失效**）；④ mock 类夹具的页数与用量合计算术（`formatBytes` <1000 不带 k）；⑤ `MIGRATION.md` 档位表与 `README.md` 的区间描述 | `bun run typecheck` + `bun run test:blueprint` + `bun run test:pages` + `bun run test:tui` + `bun run mock:wiki` + `bun run test` | 增删基础分类必须同步：① sections 区间下限（`judgeQuantity` 判的是归一化后的 count，N 个基础分类则下限随 N）；② `OVERVIEW_SECTIONS` 的 Mermaid 强制角色；③ 测试夹具里所有**靠「基础分类 + 领域」凑越界**的清单（如 `OVER_SECTIONS`，少一个基础分类要补一个领域才能继续触发越界路径，否则缩编链路**静默失效**）；④ mock 类夹具的页数与用量合计算术（`formatBytes` <1000 不带 k）；⑤ `MIGRATION.md` 档位表与 `README.md` 的区间描述 | `bun run typecheck` + `bun run test:blueprint` + `bun run test:pages` + `bun run test`（配置界面改动另跑 `bun run test:tui`） | 五档区间 / 反馈与策略文案 / 越界不落盘 / 缩编与代码兜底必须补 `test:blueprint` 断言；minimal 全景导览传递补 `test:pages`；工具名 `submit_condensed_*` 不要随意改（mock 依赖） |
...
| 一致性校验 / 黄金值对照（`tools/golden-parity-gen.py`、`packages/orchestrator/test/golden-parity.ts`、`content-gate.ts` 的 `countCjkChars` / `numbersIn`） | `python3 tools/golden-parity-gen.py` 重新生成 + `bun run typecheck` + `bun run test:golden-parity` + `bun run test` | 样本变动必须**两边同步**（Python 脚本与 TS 测试逐字一致）并重新生成黄金值；判定语义变动须在 MIGRATION 声明为有意偏差；带 `g` 标志的正则不得用于逐元素 `test()`（`lastIndex` 状态会跳过元素） |
...
- **plan.md 的五个阶段已全部落地，且已做过一轮 plan 逐条复执行**（v1.18.1）：
  内容密度门（v1.14.0）、verify-wiki 交付闸门（v1.15.0）、溯源台账（v1.16.0）、
  页面格式资产 + reader-first（v1.17.0）、标题诊断信号（v1.18.0）。
  见 `MIGRATION.md` §29–§33（§32.4 记录了复执行补齐的三项：§4 反注水清单、
  §5.5 黄金值对照、§6 mock 自动 verify）。这些改动的**真机长期使用**
...
- **「快速开始」强制基础分类已移除**（v1.19.0，五阶段之外的取向修正）：
  ...
  踩到的硬伤见 `MIGRATION.md` §34。wiki 由此不再提供「怎么跑起来」的入口，
...
- **会话已 durable，但「中断续跑」未接线**（v1.21.0 起）：方案 C 之后 pi 会话落
  ...
  （pi 侧是 JSONL 会话树 + SQLite）。见 `MIGRATION.md` §36.6。
...
- **浏览站轨迹页在运行中不再有 partial 流式预览**（方案 C，v1.21.0）：
  ...
  可改为尾随解析 append-only 的会话文件，不用回到双写。见 `MIGRATION.md` §36。
