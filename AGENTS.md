...
| 内容密度门（`wiki/content-gate.ts`、`tools/page-tools.ts`、`generate-wiki.ts` 降级落盘、`QualityConfig` 等） | `typecheck` + `test:pages` + `test:catalog` + `test:tui` + `test`；**降级落盘必须覆盖 `try` 与 `catch` 两条路径**（预算耗尽走 `catch`）；门限判定语义变动须声明为有意偏差；工具名 / schema / 提示文本不要改 |
