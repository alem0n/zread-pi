/**
 * 中文翻译字典
 */

import type { TranslationKeys } from '../types';

export const zhCN: TranslationKeys = {
  cli: {
    version: '显示版本号',
    help: '显示帮助信息',
    dirDesc: '指定目标目录（默认: 当前目录）',
    dirInvalid: '目标目录不存在或不是目录: {path}',
    wikiDesc: 'Wiki 文档生成',
    configDesc: '交互式配置编辑器',
    browseDesc: '启动 Wiki 预览服务器',
    historyDesc: '查看全局记忆（清理失效项目并列出最近生成过文档的项目）',
    historyConcurrencyDesc: '检查记录存在的并发数（默认 8）',
    logviewDesc: '启动轨迹（Trajectory）检查视图，回放本次 / 历次运行的完整事件流',
    verifyDesc: '校验已生成的 Wiki（结构 / 内容密度 / Mermaid / 溯源 / frontmatter / 覆盖等式）',
    verifyDetailDesc: '指定校验的档位（minimal/low/medium/high/max；缺省自动解析）',
    verifyEnforceDesc: '内容密度门未达标计为失败（缺省只列出）',
  },
  history: {
    pruned: '已清理 {count} 条失效记录',
    remaining: '剩余 {count} 条记录：',
    empty: '暂无历史记录',
    failed: '历史记录读取失败: {error}',
  },
  verify: {
    skipped: 'SKIP {group} {message}',
    legacy: '遗留目录（无档位子目录），页面位于 .zread-pi/wiki/',
    variant: '校验变体：{variant}',
    overallPass: 'OVERALL PASS',
    overallFail: 'OVERALL FAIL',
    detailsPrefix: '  - ',
    summary: '共 {checks} 项检查：{pass} 通过 / {fail} 失败 / {skip} 跳过',
  },
  versionGuard: {
    incompatible: '检测到不兼容的版本数据：项目家目录由 {stored} 创建，当前版本 {current}。已备份到：',
    failed: '版本守卫未完成：需要备份旧数据，但该目录正被其他程序占用，无法重命名。错误：{error}',
    failedHint: '请关闭可能占用该目录的程序（例如以该目录为工作目录的编辑器 / 终端 / AI 编程助手，或正在扫描的杀毒软件），然后重新运行 zread-pi。旧数据已保持不动，不会丢失。',
    backupHint: '旧数据已完整保留在备份目录，请尽快处理（迁移需要的文件后删除该备份目录，避免占用磁盘）。',
    created: '已初始化项目家目录（版本 {current}）。',
  },
  layout: {
    model: '模型',
    intro: '将本地代码库转化为可读的 Wiki 文档。',
    docs: '文档',
    docsNone: '未生成',
    docsGenerated: '已生成',
    docsUnit: '篇',
    docsTarget: '目标',
  },
  config: {
    title: 'Zread — 编辑配置',
    selectLanguage: '界面语言',
    docLanguage: '文档生成语言',
    llmProvider: 'LLM 提供商',
    thinkingLevel: '思考深度',
    modelSize: '模型上下文/输出',
    maxTurns: '最大轮次',
    polish: '文风润色',
    blueprintDetail: '蓝图细节档位',
    quality: '内容质量门',
    maxConcurrency: '最大并发数',
    maxRetries: '最大重试次数',
    default: '(默认: {default})',
    notConfigured: '未配置',
    footer: 'ESC 退出 | ↑↓ 选择 | Enter 确认 | s 保存',
    saved: '配置已保存',
    saveFailed: '保存失败',
    hasUnsavedChanges: '有未保存的修改',
    pressS: '按 s 保存',
    current: '当前值',
    range: '范围',
    invalidRange: '输入超出范围',
  },
  language: {
    select: '选择界面语言',
    zh: '中文',
    en: '英文',
    current: '当前',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | s 保存并返回',
  },
  docLanguage: {
    select: '选择文档生成语言',
    zh: '中文',
    en: '英文',
    current: '当前',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | s 保存并返回',
  },
  provider: {
    select: '选择 LLM 提供商',
    search: '搜索 (按 /)',
    custom: '新建自定义 Provider...',
    refresh: '刷新列表',
    loading: '正在加载 Provider 列表...',
    current: '当前',
    footer: '↑↓ 导航 | enter 选择 | / 搜索 | r 刷新 | esc 返回',
    error: '加载失败',
    configured: '已配置',
    notConfigured: '未配置',
    models: '{count} 个模型',
    authApiKey: 'API Key',
    authOauth: 'OAuth',
    source: '来源: {source}',
    customBadge: '自定义',
  },
  model: {
    select: '选择模型',
    custom: '添加自定义模型...',
    loading: '正在加载模型列表...',
    tokens: 'tokens',
    supports: '支持',
    footer: '↑↓ 导航 | enter 选择 | r 刷新模型 | a 自定义模型 | esc 返回',
    noModels: '暂无预置模型，可添加自定义模型',
    customInput: '自定义模型名称',
    count: '共 {count} 个模型',
    customBadge: '自定义',
    reasoning: '思考',
    vision: '图片',
    refreshing: '正在刷新模型列表...',
    refreshDone: '模型列表已刷新',
    refreshUnsupported: '该提供商使用内置模型目录，无需刷新',
    refreshFailed: '刷新失败: {error}',
  },
  apikey: {
    input: '输入 API Key',
    placeholder: 'sk-...',
    hidden: '(已设置)',
    required: 'API Key 不能为空',
    footer: 'enter 保存 | esc 返回',
    saved: '配置已保存',
    saving: '正在保存...',
  },
  auth: {
    loggingIn: '正在登录...',
  },
  detail: {
    apiKeyTitle: 'API Key 配置',
    modelsTitle: '模型',
    keyPlaceholderConfigured: '已设置，输入新的 API Key 可覆盖',
    keySaved: 'API Key 已保存',
    keyFailed: '保存失败: {error}',
    keyFirst: '请先配置 API Key',
    oauthOnly: '该提供商仅支持 OAuth 订阅登录（配置界面未提供，可在 auth.json 手动配置后使用）',
    ambient: '使用环境变量/外部凭据',
    currentSet: '已设为当前模型: {model}',
    footer: 'tab 切换到 API Key | ↑↓ 导航 | enter 选择模型 | r 刷新模型 | a 自定义模型 | esc 返回',
    footerCustom: 'tab 切换到 API Key | ↑↓ 导航 | enter 选择模型 | r 刷新模型 | a 自定义模型 | e 编辑 Provider | esc 返回',
    keyFooter: 'enter 保存 API Key | tab / ↑ 切换到模型 | esc 返回模型列表',
  },
  customModel: {
    title: '为该提供商添加自定义模型',
    id: '模型 ID',
    idPlaceholder: '例如 gpt-4o-mini',
    name: '显示名称（可选）',
    namePlaceholder: '例如 GPT-4o Mini',
    contextWindow: '上下文窗口 (tokens)',
    contextPlaceholder: '128000',
    maxTokens: '最大输出 (tokens)',
    maxTokensPlaceholder: '16384',
    reasoning: '支持思考',
    vision: '支持图片输入',
    xhigh: 'xhigh 超深度思考',
    max: 'max 最大思考',
    extendedHint: 'x 切换 xhigh | m 切换 max（pi 扩展档，需模型支持；未开启时由 pi 自动钳制）',
    toggleHint: 't 切换思考 | v 切换图片',
    api: '协议',
    idRequired: '模型 ID 不能为空',
    invalidNumber: '请输入正整数',
    footer: 'enter 保存 | esc 返回',
    saved: '自定义模型已添加',
  },
  customProvider: {
    createTitle: '新建自定义 Provider',
    editTitle: '编辑自定义 Provider',
    editingId: 'Provider ID（不可修改）',
    name: 'Provider 显示名称',
    namePlaceholder: '例如：我的 OpenAI 代理',
    nameRequired: 'Provider 名称不能为空',
    baseUrl: '输入 Base URL',
    baseUrlPlaceholder: 'https://api.example.com/v1',
    urlRequired: 'Base URL 不能为空',
    invalidUrl: 'URL 格式无效，请输入有效的 http/https URL',
    api: 'API 协议',
    apiHint: '当前: {api} | t 切换协议 | enter 下一步',
    footer: 'enter 下一步 | esc 返回上一步',
    step: '步骤 {current}/{total}',
  },
  concurrency: {
    set: '设置最大并发数',
    range: '范围: 1-10',
    invalid: '请输入 1-10 之间的整数',
    current: '当前值',
    footer: 'ESC 返回 | Enter 确认 | s 保存并返回',
  },
  retry: {
    set: '设置最大重试次数',
    range: '范围: 0-5',
    invalid: '请输入 0-5 之间的整数',
    current: '当前值',
    footer: 'ESC 返回 | Enter 确认 | s 保存并返回',
  },
  thinking: {
    title: '设置思考深度（pi thinking level）',
    current: '当前值',
    modelUnset: '尚未选择模型：全部等级可选，请求时按模型能力自动调整',
    supported: '当前模型支持: {levels}',
    unsupportedLevel: '当前模型不支持，请求时自动调整',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | s 保存并返回',
    levels: {
      off: '关闭',
      minimal: '最低',
      low: '低',
      medium: '中',
      high: '高',
      xhigh: '极高',
      max: '最高',
    },
  },
  maxTurns: {
    set: '设置最大轮次',
    range: '范围: 0-100（0 = 不限制）',
    hint: '到达上限前会提示模型立即输出，并自动追加 1 轮收尾轮；0 = 不限制轮次',
    invalid: '请输入 0-100 之间的整数',
    current: '当前值',
    footer: 'ESC 返回 | Enter 确认 | s 保存并返回',
  },
  modelSize: {
    title: '设置模型上下文窗口 / 最大输出',
    modelUnset: '尚未选择模型：覆盖值仍会生效，默认值参考目录回退值（200000 / 8192）',
    defaults: '模型目录默认: 上下文 {context} · 最大输出 {maxTokens}',
    hint: '留空 = 跟随模型目录默认；显式值会覆盖目录元数据（影响请求输出上限与上下文压缩阈值）',
    contextWindow: '上下文窗口 (tokens)',
    maxTokens: '最大输出 (tokens)',
    invalidContext: '请输入 {min}-{max} 之间的正整数',
    invalidMaxTokens: '请输入 {min}-{max} 之间的正整数',
    followDefault: '跟随模型默认',
    asDefault: '默认',
    footer: 'tab / ↑↓ 切换 | Enter 确认并返回 | d 恢复模型默认 | s 保存并返回 | ESC 返回',
  },
  polish: {
    title: '设置文风纪律与页面润色',
    current: '当前值',
    intro:
      '第 1 层预防（默认）：把文风纪律注入系统提示，零额外成本；第 2 层兜底（full）：每页落盘后额外跑一次润色 Agent，效果更好但每页多一次 LLM 调用。',
    enabled: '已启用',
    disabled: '已停用',
    modePromptOnly: '仅提示注入',
    modePromptOnlyDesc: '第 1 层预防：只把文风纪律拼进系统提示，不增加任何 LLM 调用',
    modeFull: '完整模式',
    modeFullDesc: '预防 + 第 2 层兜底：每页落盘后额外跑一次润色 Agent（失败不判页失败）',
    layersHint:
      '两种模式下，代码块、行内代码、Sources: 溯源行、Mermaid 引号标签与 YAML frontmatter 都受保护。',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | t 启用/停用 | s 保存并返回',
  },
  quality: {
    title: '设置内容质量门',
    current: '当前值',
    intro:
      '内容密度门把「页面是否只有干瘪 TL;DR」变成机械可判定的指标（散文篇幅 / 标题层级 / 溯源行 / Mermaid / 代码块 / 句首重复）。下限按难度与关联文件数自适应。',
    enabled: '已启用',
    disabled: '已停用',
    modeOff: '关闭',
    modeOffDesc: '完全不校验（行为与升级前一致）',
    modeWarn: '仅告警（默认）',
    modeWarnDesc: '计算并记录进页面结果，不拦截落盘，页面照常生成',
    modeEnforce: '强制达标',
    modeEnforceDesc: '未达标时拦截并要求重写；预算用尽仍未通过则 best-effort 落盘并标记告警',
    verifyAfterGenerate: '生成后自动校验',
    verifyAfterGenerateDesc: '生成完成后自动跑一次交付闸门（verify-wiki），默认关闭',
    paddingHint:
      '门限是下限不是目标：不要同义改写注水、不要为凑图表而加图；源里没有可写代码时代码块正确答案是 0。',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | t 启用/停用 | s 保存并返回',
  },
  blueprintDetail: {
    title: '设置蓝图细节档位',
    current: '当前值',
    intro:
      '档位决定结构层的目标参数（分类数 / 每分类文章数）；区间是目标不是硬约束——结构层按代码库的实际结构选层，LLM 只负责命名。',
    minimal: '极简',
    minimalDesc: '1 个分类（概览）· 1 篇全景导览：必须用 Mermaid 架构图梳理模块关系与数据流（跳过命名 Agent）',
    low: '精简',
    lowDesc: '3~5 个分类 · 每分类 1~3 篇',
    medium: '标准',
    mediumDesc: '4~6 个分类 · 每分类 3~5 篇',
    high: '详细（默认）',
    highDesc: '4~8 个分类 · 每分类 3~10 篇（老用户零变化）',
    max: '最详尽',
    maxDesc: '4~8 个分类 · 每分类 5~12 篇，强调全面详尽并鼓励深挖关联文件',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | s 保存并返回',
  },
  tools: {
    title: '外部工具',
    homeLabel: '外部工具',
    intro: '安装 ripgrep / fd 会让 grep 与 find 更快；未安装时自动使用内置实现，功能不受影响。',
    ready: '就绪',
    readyRatio: '{ready}/{total} 就绪',
    stateSystem: '系统已安装',
    stateManaged: '已由 zread-pi 安装',
    stateMissing: '未安装',
    stateDisabled: '已停用',
    fieldState: '状态',
    fieldVersion: '版本',
    fieldPath: '路径',
    fieldUsedBy: '用于',
    fieldManagedDir: '安装目录',
    fieldEnabled: '启用状态',
    notInstalled: '未安装',
    versionUnknown: '未识别（不影响使用）',
    versionMismatch: '台账版本 {expected}，实际探测到 {actual}',
    probeOutput: '探测输出',
    probeArgs: '探测参数',
    managedHint: '按 d 可卸载（仅删除 zread-pi 安装的副本）',
    systemHint: '使用系统 PATH 中的版本；需要在 zread-pi 内管理时可覆盖安装',
    missingHint: '按 Enter 下载并安装；也可以按住 t 停用以强制使用内置实现',
    disabledHint: '已停用：即使本机安装也不会使用，grep / find 走内置实现',
    notInstallable: '当前平台不支持自动安装，请手动安装后重新打开此页',
    enabled: '已启用',
    disabled: '已停用',
    phaseResolving: '解析版本',
    phaseDownloading: '下载中',
    phaseExtracting: '解包中',
    phaseVerifying: '校验中',
    phaseDone: '完成',
    installing: '正在安装…',
    installDone: '安装完成',
    installDoneNoVersion: '安装完成（版本输出未识别，已记录安装版本 {version}）',
    installFailed: '安装失败',
    uninstallDone: '已卸载',
    uninstallNothing: '没有可卸载的 zread-pi 安装副本',
    unsavedHint: '有未保存的修改 · 按 s 保存',
    listFooter: 'ESC 返回 | ↑↓ 选择 | Enter 管理',
    detailFooter: 'ESC 返回 | Enter 安装/重装 | d 卸载 | t 启用/停用 | s 保存并返回',
    busyFooter: '安装中… 请稍候（ctrl+c 退出）',
    usage: {
      rg: 'grep（文件内容搜索）',
      fd: 'find（文件名搜索）',
    },
  },
  common: {
    escBack: 'ESC 返回',
    saveAndBack: 's 保存并返回',
  },
  divider: {
    prefix: '── ',
    middle: ' ─',
  },
  wiki: {
    title: 'Zread — Wiki 文档生成',
    generate: '生成文档',
    continue: '继续生成 ({generated}/{total})',
    manage: '管理文档',
    browse: '浏览文档',
    force: '强制重新生成',
    logview: '运行轨迹',
    firstTimeConfig: '配置',
    config: '配置',
    exit: '退出',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认 | l 运行轨迹',
    // Divider 状态标题
    dividerFirstTime: '尚未配置 LLM 提供商',
    dividerNoCatalog: '尚无文档目录',
    dividerHasCatalog: '目录已生成',
    dividerInProgress: '文档生成中 ({generated}/{total})',
    dividerComplete: '文档已生成 ({total} 篇)',
    // Sync
    sync: '同步文档',
    dividerHasSync: 'wiki 变更扫描',
  },
  browse: {
    title: 'Zread — Wiki 文档浏览',
    starting: '启动服务器...',
    running: '服务器已启动',
    url: '访问地址',
    footer: 'ESC 退出服务器 | ctrl+c 退出应用',
    stopped: '服务器已停止',
    startFailed: '启动服务器失败',
    noDocs: '暂无文档，请先生成',
  },
  logview: {
    title: 'Zread — 运行轨迹',
    starting: '启动服务器...',
    running: '轨迹视图已启动',
    url: '访问地址',
    footer: 'ESC 退出服务器 | ctrl+c 退出应用',
    stopped: '服务器已停止',
    startFailed: '启动服务器失败',
    noRuns: '暂无运行记录',
    noRunsHint: '生成或同步一次文档后，这里会留下可回放的运行轨迹',
  },
  wikiGenerate: {
    catalogTitle: '目录',
    articlesTitle: '文章 {current}/{total}',
    waiting: '等待',
    requesting: '请求中',
    responding: '响应中',
    tool: '工具: {name}',
    retrying: '重试 {n}/{max} · {seconds}s',
    completed: '完成',
    failed: '失败',
    navigate: '导航',
    viewTrajectory: '查看运行轨迹',
    retry: '重新生成',
    exit: '退出',
    usageTotals: '合计 输入 {input} · 输出 {output} · 缓存占比 {ratio}',
    // 逐 Agent 行：四个状态指标（输入 / 输出 / 缓存占比 / 上下文占比）
    metricsCache: '缓存占比 {ratio}',
    metricsContext: '上下文 {used}/{window} ({ratio})',
    // 目录 Agent 行标签（每个 Agent 一行；名称描述该 Agent 此刻在做什么）
    agentClassify: '规划主题',
    agentTopics: '拟定标题 · {section}',
    agentTitles: '精修标题 · {section}',
    agentCondenseSections: '精简主题',
    agentCondenseTopics: '精简标题 · {section}',
    // 结构优先蓝图的新角色（旧键保留，供旧日志回放）
    agentStructure: '结构切分',
    agentSections: '命名分类',
    agentPages: '命名页面 · {section}',
    // 三阶段进度：与 Agent 行同词，避免同一阶段出现两种叫法
    stageClassify: '规划主题中',
    stageTopics: '拟定标题 {current}/{total}',
    stageTitles: '精修标题 {current}/{total}',
    stageTopicsIdle: '拟定标题中',
    stageTitlesIdle: '精修标题中',
    // 结构优先蓝图的新阶段（旧键保留，供旧日志回放）
    stageStructure: '结构切分中',
    stageSections: '命名分类中',
    stagePages: '命名页面 {current}/{total}',
    stagePagesIdle: '命名页面中',
    stageSection: ' · {section}',
    failedSections: '{n} 个分类失败',
    gateWarn: '密度门未达标',
    gateDegraded: '密度门降级落盘',
  },
};