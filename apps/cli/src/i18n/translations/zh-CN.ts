/**
 * 中文翻译字典
 */

import type { TranslationKeys } from '../types';

export const zhCN: TranslationKeys = {
  cli: {
    version: '显示版本号',
    help: '显示帮助信息',
    wikiDesc: 'Wiki 文档生成',
    configDesc: '交互式配置编辑器',
    browseDesc: '启动 Wiki 预览服务器',
  },
  layout: {
    provider: '提供商',
    model: '模型',
    baseUrl: 'Base URL',
    directory: '目录',
    intro: '将本地代码库转化为可读的 Wiki 文档。',
    github: '开源项目: {url}',
  },
  config: {
    title: 'Zread — 编辑配置',
    selectLanguage: '界面语言',
    docLanguage: '文档生成语言',
    llmProvider: 'LLM 提供商',
    maxConcurrency: '最大并发数',
    maxRetries: '最大重试次数',
    default: '(默认: {default})',
    notConfigured: '未配置',
    footer: 'ESC 退出 | ↑↓ 选择 | Enter 确认 | s 保存',
    saved: '配置已保存',
    saveFailed: '保存失败',
    hasUnsavedChanges: '有未保存的修改',
    pressS: '按 s 保存',
    current: '当前值',
    range: '范围',
    invalidRange: '输入超出范围',
  },
  language: {
    select: '选择界面语言',
    zh: '中文',
    en: '英文',
    current: '当前',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | s 保存并返回',
  },
  docLanguage: {
    select: '选择文档生成语言',
    zh: '中文',
    en: '英文',
    current: '当前',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认并返回 | s 保存并返回',
  },
  provider: {
    select: '选择 LLM 提供商',
    search: '搜索 (按 /)',
    custom: '自定义 Provider...',
    refresh: '刷新列表',
    loading: '正在加载 Provider 列表...',
    current: '当前',
    footer: '↑↓ 导航 | enter 选择 | / 搜索 | r 刷新 | esc 返回',
    error: '加载失败',
    configured: '已配置',
    notConfigured: '未配置',
    models: '{count} 个模型',
    authApiKey: 'API Key',
    authOauth: 'OAuth',
    source: '来源: {source}',
    customBadge: '自定义',
  },
  model: {
    select: '选择模型',
    custom: '添加自定义模型...',
    loading: '正在加载模型列表...',
    tokens: 'tokens',
    supports: '支持',
    footer: '↑↓ 导航 | enter 选择 | r 刷新模型 | a 自定义模型 | esc 返回',
    noModels: '暂无预置模型，可添加自定义模型',
    customInput: '自定义模型名称',
    count: '共 {count} 个模型',
    customBadge: '自定义',
    reasoning: '思考',
    vision: '图片',
    refreshing: '正在刷新模型列表...',
    refreshDone: '模型列表已刷新',
    refreshUnsupported: '该提供商使用内置模型目录，无需刷新',
    refreshFailed: '刷新失败: {error}',
  },
  apikey: {
    input: '输入 API Key',
    placeholder: 'sk-...',
    hidden: '(已设置)',
    required: 'API Key 不能为空',
    footer: 'enter 保存 | esc 返回',
    saved: '配置已保存',
    saving: '正在保存...',
  },
  auth: {
    chooseType: '选择登录方式',
    useOAuth: '账号登录 (OAuth)',
    useApiKey: 'API Key 登录',
    apiKeyLabel: 'API Key',
    oauthLabel: 'OAuth',
    oauthHint: '订阅制账号（Claude Pro/Max、ChatGPT、Copilot 等）',
    currentModel: '当前使用此模型',
    loggingIn: '正在登录...',
    waiting: '等待浏览器授权...',
    openUrl: '请在浏览器打开: {url}',
    deviceCode: '设备码: {code} · 授权地址: {uri}',
    success: '登录成功',
    failed: '登录失败: {error}',
    login: '登录',
    relogin: '重新登录',
    logout: '退出登录',
    loggedOut: '已退出登录',
    logoutFailed: '退出登录失败: {error}',
    footerChoose: '↑↓ 选择 | enter 确认 | esc 返回',
    footerApiKey: 'enter 登录 | esc 返回',
    footerConfigured: 'enter 使用此模型 | l 重新登录 | d 退出登录 | esc 返回',
  },
  customModel: {
    title: '为该提供商添加自定义模型',
    id: '模型 ID',
    idPlaceholder: '例如 gpt-4o-mini',
    name: '显示名称（可选）',
    namePlaceholder: '例如 GPT-4o Mini',
    contextWindow: '上下文窗口 (tokens)',
    contextPlaceholder: '128000',
    maxTokens: '最大输出 (tokens)',
    maxTokensPlaceholder: '16384',
    reasoning: '支持思考',
    vision: '支持图片输入',
    toggleHint: 't 切换思考 | v 切换图片',
    api: '协议',
    idRequired: '模型 ID 不能为空',
    invalidNumber: '请输入正整数',
    footer: 'enter 保存 | esc 返回',
    saved: '自定义模型已添加',
  },
  customProvider: {
    title: '自定义 Provider 配置',
    baseUrl: '输入 Base URL',
    baseUrlPlaceholder: 'https://api.example.com/v1',
    modelName: '输入模型名称',
    modelNamePlaceholder: 'model-name',
    apikey: '输入 API Key',
    invalidUrl: 'URL 格式无效，请输入有效的 http/https URL',
    footer: 'enter 下一步 | esc 返回上一步',
    step: '步骤 {current}/{total}',
  },
  concurrency: {
    set: '设置最大并发数',
    range: '范围: 1-10',
    invalid: '请输入 1-10 之间的整数',
    current: '当前值',
    footer: 'ESC 返回 | Enter 确认 | s 保存并返回',
  },
  retry: {
    set: '设置最大重试次数',
    range: '范围: 0-5',
    invalid: '请输入 0-5 之间的整数',
    current: '当前值',
    footer: 'ESC 返回 | Enter 确认 | s 保存并返回',
  },
  common: {
    escBack: 'ESC 返回',
    saveAndBack: 's 保存并返回',
  },
  divider: {
    prefix: '── ',
    middle: ' ─',
  },
  wiki: {
    title: 'Zread — Wiki 文档生成',
    generate: '生成文档',
    continue: '继续生成 ({generated}/{total})',
    manage: '管理文档',
    browse: '浏览文档',
    force: '强制重新生成',
    firstTimeConfig: '配置',
    config: '配置',
    exit: '退出',
    footer: 'ESC 返回 | ↑↓ 选择 | Enter 确认',
    // Divider 状态标题
    dividerFirstTime: '尚未配置 LLM 提供商',
    dividerNoCatalog: '尚无文档目录',
    dividerHasCatalog: '目录已生成',
    dividerInProgress: '文档生成中 ({generated}/{total})',
    dividerComplete: '文档已生成 ({total} 篇)',
    // Sync
    sync: '同步文档',
    dividerHasSync: 'wiki 变更扫描',
  },
  browse: {
    title: 'Zread — Wiki 文档浏览',
    starting: '启动服务器...',
    running: '服务器已启动',
    url: '访问地址',
    footer: 'ESC 退出服务器 | ctrl+c 退出应用',
    stopped: '服务器已停止',
    noDocs: '暂无文档，请先生成',
  },
  wikiGenerate: {
    catalogTitle: '目录',
    articlesTitle: '文章 {current}/{total}',
    waiting: '等待',
    requesting: '请求中',
    responding: '响应中',
    tool: '工具: {name}',
    retrying: '重试 {n}/{max} · {seconds}s',
    completed: '完成',
    failed: '失败',
    navigate: '导航',
    retry: '重新生成',
    exit: '退出',
  },
};