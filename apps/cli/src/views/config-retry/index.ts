/**
 * Config Retry Page - 最大重试次数设置（pi-tui 版）
 *
...
export default class ConfigRetryPage extends Screen {
  ...
  protected override init(): void {
    const current = String(this.app.config.config.concurrency.max_retries);
  ...
  render(width: number): string[] {
    ...
    // Footer（marginTop={1}）
    lines.push("", style(this.t("retry.footer"), { dim: true }));
  ...
  private validate(value: string): boolean {
    ...
    if (Number.isNaN(num) || num < MIN || num > MAX) {
      this.error = this.t("retry.invalid");
      this.refresh();
      return false;
    }
    this.error = null;
