...
你的代码库
   ...
   │                ├─ 分类   1 个 Agent 划分 4~8 个顶级章节（含概览/快速开始/核心架构）
...
1. **分类。** 一个 Agent 只看架构，产出 4~8 个顶级章节（强制包含概览 / 快速开始 / 核心架构）；
...
`minimal` 只产 1 个分类 + 1 篇全景导览（必须 Mermaid 架构图），`low` 为 3~5 分类且跳过标题精修。
...
```text
~/.zread-pi/logs/zread-pi-<yyyy-MM-dd>.log
```

每行格式为 `[本地时间] [级别] 模块名 消息`，模块名是点号分层的（如 `orchestrator.pages`、`analyzer.scanner`），
便于按子系统过滤。默认保留 30 天。
