...
- **结构优先蓝图** —— 目录结构由代码定：CEG（跨文件依赖图）→ 确定性两级切分（图划分 → 文件切片）→ 槽位（section / page 骨架，含 ownsFiles 归属与缝合线），LLM 只负责**命名**（标题 / 描述 / scope / summary），不再让模型决定「哪个文件进哪页」，结构漂移与数量偏少问题从根上消失。
- **五档蓝图细节（`blueprint.detail`）** —— 从 `minimal`（1 个分类 · 1 篇全景导览，必须 Mermaid 架构图，适合快速了解）到 `max`（每分类 5~12 篇、深挖关联文件），默认 `high` 与旧行为一致；档位是结构层的**目标参数**（选层窗口 / 切片粒度），区间是目标不是硬约束，生成永不悬挂。
...
- **生成过程可见的用量账本** —— 生成页每个条目（目录展开为每个 Agent 一行：结构切分 / 分类命名 / 每个分类的页面命名；文章列表每篇一行）右侧都实时显示输入 token、输出 token、缓存读占比与当前上下文占比（已用 / 窗口），完成后也继续显示；底部另有全部 Agent 的合计，口径来自 pi 的 usage 账本（输入侧含缓存读写），按 `r` 重新生成不会清空已消耗的用量，多页并发生成时合计也不会重复计数。
...
- **交付闸门** —— `zread-pi verify` 一条命令判定本次生成是否达标（结构 / 内容密度 / Mermaid / 溯源 / frontmatter / **覆盖等式**），逐行 `PASS`/`FAIL`/`SKIP` + 退出码，可直接进 CI；覆盖等式独立重算「每个文件的每一行是否恰好归入一个页面」（行级台账 + ownsFiles 一致性 + 排除项清单），溯源会核对页面声称的路径、行号与行内代码符号是否真实存在于源码（符号级只警告、不判失败），生成后也能自动跑一次并落 `verify.json`（`quality.verifyAfterGenerate`）。
...
# 2) 离线全链路试跑：用 mock LLM 对夹具仓库跑「扫描 -> 结构 -> 命名 -> 并行页面」
bun run mock:wiki
...
| `zread-pi verify [--detail <档位>] [--enforce]` | 交付闸门：逐条输出 `PASS`/`FAIL`/`SKIP`（结构 / 内容密度 / Mermaid / 溯源 / frontmatter / 覆盖等式），末尾 `OVERALL PASS`/`FAIL`，退出码随之；覆盖等式独立重算行级台账（每行恰好归入一个页面、ownsFiles 一致、排除项 = 清单 − 未解析文件）；溯源含路径真实、行号有效、行内代码与图表符号（序列图参与者 / 消息标签 / 状态图状态名）可溯（WARN）与蓝图 `associatedFiles` 存在性；`--enforce` 才把内容密度门未达标计为失败，否则只列出 |
...
你的代码库
   ...
   │
   ├── 4. 结构    CEG 跨文件依赖图 → 确定性两级切分（划分 → 切片）
   │           → section / page 槽位骨架（每页 ownsFiles 归属 + 缝合线），
   │           落盘审计件 cache/structure-<档位>.json（每次运行重算，可复现）
   │
   ├── 5. 命名    只给骨架起名字（代码决定结构，LLM 只填名字）：
   │           ├─ 分类命名 1 个 Agent 填 section 标题 / 描述 / scope
   │           └─ 页面命名 每个分类 1 个 Agent 并行填 page 标题 / summary
   │
   └── 6. 生成    N 个并行页面 Agent 只读自己 ownsFiles 的真实代码，
...
让 LLM 决定「哪个文件进哪页」有两个治不好的毛病：**漂移**（同一模块的文件被拆进多个章节、
边界与 scope 对不上）与**偏少**（输出轮次一多模型就提前收敛，一个分类只写一两篇就“完成”）。

zread-pi 把蓝图拆成两条链：

1. **结构链（代码，零 LLM）。** 扫描 + 解析后构建 CEG（跨文件依赖图），跑一次**确定性**
   两级切分——先图划分得到 section 骨架，再在 section 内把文件切成 page 槽位，并为每个槽位
   算出 `ownsFiles`（本页拥有的文件 / 行范围）与**缝合线**（跨 section 的引用，需在页面里说明）；
   骨架直接落盘 `cache/structure-<档位>.json`，同一仓库同一档位重算字节级一致，可复现、可审计。
2. **命名链（LLM，只填名字）。** 分类命名 Agent 只给 section 起标题 / 描述 / scope；
   每个分类一个页面命名 Agent 并行给自己的 page 槽位起标题与一句 `summary`（论证什么、
   以哪些文件为证据）。因为结构已定，模型无法再“少写几页”，也无法把别处的文件顺手写进来。

档位（`blueprint.detail`，默认 `high` = 3~8 分类 / 每分类 3~10 篇）是结构层的**目标参数**：
`sections` 区间是选层窗口、`topics` 区间决定切片粒度；区间是目标不是硬约束——
结构层按代码库的实际结构选层，实际数量可能落在区间外，`minimal` 甚至跳过两个命名 Agent
（1 个分类 + 1 篇全景导览，必须 Mermaid 架构图），零 LLM 请求出蓝图。

命名失败不阻断：命名 Agent 出错时沿用机器标题（`section-<n>` / `page-<slug>`），
页面照常生成；slug、文件编号与去重全部由代码统一分配，单个分类失败不会影响其余分类。

页面生成阶段把该页的 `ownsFiles`、跨页引用（路径 / 原因 / 负责该路径的页面 slug）
与触及本页的**缝合线**注入写作提示词，并附一行范围纪律（内容不得越出拥有的范围）——
这些边界是结构层算出来的硬事实，不是模型自己声明的，因此可直接校验：

- `zread-pi verify` 的**覆盖等式**检查会独立重算行级台账：每个被解析文件的每一行
  必须恰好归入一个页面（行区间不重叠、不越界），排除项必须等于「清单 − 未解析文件」；
- 溯源检查核对页面声称的路径、行号与行内符号是否真实存在于源码（符号级只警告、不判失败）。

> [!IMPORTANT]
> v1.23.0 把蓝图从「三阶段 LLM 规划」改为「结构优先」后，`wiki.json` 的**页面结构与归属**
> 完全由代码决定。升级后旧产物需要重新生成一次（`zread-pi wiki` 全量，或 `zread-pi wiki --sync`
> 走增量身份继承：按 ownsFiles 重叠 / slot slug 把旧页面映射到新骨架，未变的页面原样保留）。

旧版 `wiki.json` 的 scope / summary 字段仍会沿用（全量重生成时由命名链重填），
缺字段时按「无边界」降级，不阻断生成。

<details>
<summary><strong>交付闸门长什么样？（点击展开真实输出）</strong></summary>

`zread-pi verify` 逐条检查、给出 `PASS` / `FAIL` / `SKIP` 与退出码，可直接进 CI：

```text
▶ 交付闸门：overall=PASS（legacy=false）
   ✓ [coverage] 覆盖等式成立：3 个文件恰被一个页面拥有（|U| == Σ|ownsFiles|）
   ✓ [coverage] 文件归属排他（无文件被两页同时拥有）
   ✓ [coverage] 行台账一致：已测 3 个文件 / 共 44 行（declared 18，gap 26）
   ✓ [coverage] excluded 一致：0 个未解析文件（manifest − U）
```

覆盖等式组有四个 SKIP 分支（都只读不写产物）：

- 旧版 `wiki.json`（`schemaVersion != 2`）——结构优先之前的产物没有可校验的归属信息；
- 没有缓存清单（`last_manifest.json` 缺失）；
- 清单哈希不匹配（扫描结果变了，需先重新生成）；
- 没有任何被解析的符号。
...
3. 只对受影响的部分做增量修补：先重算结构骨架，再由 `reconcileBlueprint` 做身份继承
   （按 ownsFiles 重叠把旧页面映射到新槽位，slot 页面按 slug 精确匹配），
   只对新增 / 归属变化的分类与页面调命名 Agent；未受影响的页面原样保留，
...
**蓝图细节档位区间**（`blueprint.detail`，默认 `high`；配置界面 `/config/detail`；区间是**目标参数**，不是硬约束）：

| 档位 | 分类数（目标） | 每分类文章数（目标） | 附加 |
 --- | --- | --- | --- |
| `minimal` | 固定 1（概览） | 固定 1 | 全景导览（必须 Mermaid 架构图），跳过两个命名 Agent |
| `low` | 2~5 | 1~3 | — |
| `medium` | 3~6 | 3~5 | — |
| `high`（默认） | 3~8 | 3~10 | — |
| `max` | 3~8 | 5~12 | 强调全面详尽、深挖关联文件 |

> 非 minimal 档位由结构层按依赖图选层，实际分类数可能落在区间外（区间是目标不是硬约束）；
> `minimal` 的唯一页面附加「全景导览」要求。结构骨架落盘 `cache/structure-<档位>.json`，可复现审计。
...
└── .zread-pi/
    ...
    └── cache/
        ...
        └── structure-<档位>.json        # 结构骨架审计件（CEG 图 → 切分 → 槽位；每次运行重算，可复现）
```
